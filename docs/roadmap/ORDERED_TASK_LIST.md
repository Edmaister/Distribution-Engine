...
Status: Complete (2026-07-19). Output: `frontend/src/pages/admin/ReferralSaasAccountSetupPage.tsx`; `frontend/src/pages/admin/ReferralSaasAccountSetupPage.test.tsx`; `apps/api/routers/admin_onboarding.py`; `test/api/test_admin_onboarding_api.py`; `docs/roadmap/referral-saas/ROADMAP.md`; `docs/sa/referral-saas/REFERRAL_SAAS_GAP_MATRIX.md`; `docs/roadmap/ORDERED_TASK_LIST.md`.
Product boundary: Referral SaaS.
Required boundary docs checked: `AGENTS.md`; `docs/product/referral-saas/PRODUCT_BRIEF.md`; `docs/roadmap/referral-saas/ROADMAP.md`; `docs/sa/referral-saas/REFERRAL_SAAS_GAP_MATRIX.md`; `docs/roadmap/ORDERED_TASK_LIST.md`.
...
Files involved: `frontend/src/pages/admin/ReferralSaasAccountSetupPage.tsx`; `frontend/src/pages/admin/ReferralSaasAccountSetupPage.test.tsx`; `apps/api/routers/admin_onboarding.py`; `test/api/test_admin_onboarding_api.py`; `docs/roadmap/referral-saas/ROADMAP.md`; `docs/sa/referral-saas/REFERRAL_SAAS_GAP_MATRIX.md`; `docs/roadmap/ORDERED_TASK_LIST.md`.
Implementation/source files inspected: `frontend/src/pages/admin/ReferralSaasAccountSetupPage.tsx`; `frontend/src/pages/admin/ReferralSaasAccountSetupPage.test.tsx`; `apps/api/routers/admin_onboarding.py`; `test/api/test_admin_onboarding_api.py`; `docs/product/referral-saas/PRODUCT_BRIEF.md`; `docs/roadmap/referral-saas/ROADMAP.md`; `docs/sa/referral-saas/REFERRAL_SAAS_GAP_MATRIX.md`; `docs/roadmap/ORDERED_TASK_LIST.md`.
Database/schema impact: None.
Backend impact: Keeps the membership posture read API and invitation intent command available for future Account Maintenance work. Splits saved draft section handling so public draft selector readback remains allowlisted while submit-for-review and review-decision validation still inspect full saved draft evidence for blocked live-action or unsafe fields.
...
API impact: None.
Tests to add/update: Updates Account Setup tests to assert the six-step journey, direct Company Profile to Integration Intent progression, absence of People and Roles and Record role intent controls, preserved account creation guardrails, and backend submit/review blocker coverage for full saved draft evidence.
Validation method: `npm.cmd test -- ReferralSaasAccountSetupPage.test.tsx`; `npm.cmd run build`; `.venv_codex\Scripts\python.exe -m pytest -q test\api\test_admin_onboarding_api.py -k "saved_section_rows_decode_jsonb_strings_for_submit_validation or submit_for_review_validation_blockers_prevent_transition or submit_for_review_redacts_unsafe_saved_evidence_and_no_dispatch or review_decision_validation_blockers_prevent_decision or review_decision_redacts_hostile_saved_evidence_and_no_dispatch"`; `.venv_codex\Scripts\python.exe -m py_compile apps\api\routers\admin_onboarding.py test\api\test_admin_onboarding_api.py`; `git diff --check`.
Acceptance criteria: Account Setup no longer asks operators to create or plan additional users/roles; the primary setup contact remains captured in Company Profile; wizard gating works across six steps; Account Setup still shows safe membership posture on handoff when available; saved draft selector readback stays sanitized; submit/review transitions remain blocked when full saved draft evidence contains unsafe live-action fields; no membership API/service, schema, account command, campaign activation, go-live, money, or DLaaS marketplace behavior is removed or changed.
...
## TASK-227: Add Account Maintenance durable account selector

Status: Complete (2026-07-19). Output: `services/referral_saas_account_foundation_service.py`; `apps/api/routers/referral_saas_accounts.py`; `frontend/src/api/endpoints/referralSaasAccounts.ts`; `frontend/src/api/referralSaasAccountQueries.ts`; `frontend/src/pages/admin/ReferralSaasAccountMaintenancePage.tsx`; `test/test_referral_saas_account_foundation_service.py`; `test/api/test_referral_saas_accounts_api.py`; `frontend/src/api/endpoints/referralSaasAccounts.test.ts`; `frontend/src/pages/admin/ReferralSaasAccountMaintenancePage.test.tsx`; `scripts/referral_saas_route_smoke_plan.py`; `test/test_referral_saas_route_smoke_inventory.py`; `test/test_referral_saas_route_smoke_plan.py`; `docs/sa/referral-saas/REFERRAL_SAAS_ROUTE_SMOKE_INVENTORY.md`; `docs/roadmap/referral-saas/ROADMAP.md`; `docs/sa/referral-saas/REFERRAL_SAAS_GAP_MATRIX.md`; `docs/roadmap/ORDERED_TASK_LIST.md`.
Product boundary: Referral SaaS.
Required boundary docs checked: `AGENTS.md`; `docs/product/referral-saas/PRODUCT_BRIEF.md`; `docs/roadmap/referral-saas/ROADMAP.md`; `docs/sa/referral-saas/REFERRAL_SAAS_GAP_MATRIX.md`; `docs/roadmap/ORDERED_TASK_LIST.md`.
Shared primitive impact: Adds a read-only product wrapper over the existing account foundation tables so Account Maintenance can start from durable account selection while keeping account foundation, external references, tenant links, readiness, and draft evidence single-source. Source duplication: No.
Linked enhancement: Referral Management and Campaign Attribution SaaS first-wedge productization.
Linked platform/product capability: Account Maintenance account registry selector; durable account read model; customer maintenance entry point.
Objective: Let operators select the real Referral SaaS account foundation they want to maintain before reviewing readiness, access posture, technical posture, campaign handoff, reporting, or support evidence.
Why now: User testing showed Account Maintenance was still scoped by typed references and setup drafts, which made it feel like another setup screen instead of a maintenance workspace for existing accounts.
Files involved: `services/referral_saas_account_foundation_service.py`; `apps/api/routers/referral_saas_accounts.py`; `frontend/src/api/endpoints/referralSaasAccounts.ts`; `frontend/src/api/referralSaasAccountQueries.ts`; `frontend/src/pages/admin/ReferralSaasAccountMaintenancePage.tsx`; `frontend/src/pages/admin/ReferralSaasAccountMaintenancePage.test.tsx`; `docs/roadmap/referral-saas/ROADMAP.md`; `docs/sa/referral-saas/REFERRAL_SAAS_GAP_MATRIX.md`; `docs/roadmap/ORDERED_TASK_LIST.md`.
Implementation/source files inspected: `services/referral_saas_account_foundation_service.py`; `apps/api/routers/referral_saas_accounts.py`; `frontend/src/pages/admin/ReferralSaasAccountMaintenancePage.tsx`; `frontend/src/api/endpoints/referralSaasAccounts.ts`; `frontend/src/api/referralSaasAccountQueries.ts`; `docs/product/referral-saas/PRODUCT_BRIEF.md`; `docs/roadmap/referral-saas/ROADMAP.md`; `docs/sa/referral-saas/REFERRAL_SAAS_GAP_MATRIX.md`; `docs/roadmap/ORDERED_TASK_LIST.md`.
Database/schema impact: No migration changes. Reads existing TASK-198 `platform_accounts` and `platform_external_tenant_refs` rows.
Backend impact: Adds `list_referral_saas_accounts` read service and `GET /v1/referral-saas/accounts` product wrapper with reader-role guardrails, safe output redactions, limit bounds, and no mutation behavior.
Frontend impact: Adds account registry endpoint client/query hook and makes Account Maintenance start with a real account selector; manual reference lookup remains a backup path; saved setup drafts are labelled as fallback evidence for accounts not created yet.
API impact: Adds one read-only product route under `/v1/referral-saas/accounts`.
Tests to add/update: Adds service/API/frontend endpoint/page tests and updates Referral SaaS route smoke inventory/plan tests.
Validation method: `pytest -q test/test_referral_saas_account_foundation_service.py test/api/test_referral_saas_accounts_api.py test/test_referral_saas_route_smoke_inventory.py test/test_referral_saas_route_smoke_plan.py`; `npm.cmd test -- referralSaasAccounts.test.ts ReferralSaasAccountMaintenancePage.test.tsx`; `npm.cmd run build`; `git diff --check`.
Acceptance criteria: Account Maintenance lists real durable account foundations from safe product API output; selecting an account loads maintenance evidence using external customer and organisation references; setup drafts remain available only as fallback evidence; internal `tenant_code`, secrets, raw payloads, money, and DLaaS marketplace details are not exposed; no account create/update/lifecycle, membership write, invitation delivery, credential lifecycle, campaign activation, go-live, support-case write, repair/replay/retry, reward, funding, fulfilment, settlement, commission, wallet, invoice, payout, sponsor billing, treasury, or broad DLaaS marketplace behavior is added.
Dependencies: TASK-226.
Blocked by: None. Local UI-driven Account Setup wizard E2E readiness proof remains future work.
Risk level: Medium.
Rollback notes: Revert the account registry service/API/client/hook/page/test updates and roadmap/gap/task-list entries.
...
Status: Complete (2026-07-19). Output: `dp/migrations/083_referral_saas_account_operating_jurisdiction.sql`; `services/referral_saas_account_foundation_service.py`; `services/referral_saas_account_setup_service.py`; `apps/api/routers/referral_saas_accounts.py`; `frontend/src/api/endpoints/referralSaasAccounts.ts`; `frontend/src/pages/admin/ReferralSaasAccountMaintenancePage.tsx`; `frontend/src/pages/admin/ReferralSaasAccountMaintenancePage.test.tsx`; `frontend/src/app/App.tsx`; `frontend/src/layout/Sidebar.tsx`; `frontend/src/styles/base.css`; `test/test_referral_saas_account_operating_jurisdiction_migration.py`; `test/test_referral_saas_account_foundation_migration.py`; `test/test_referral_saas_account_foundation_service.py`; `test/test_referral_saas_account_setup_service.py`; `test/api/test_referral_saas_accounts_api.py`; `frontend/src/api/endpoints/referralSaasAccounts.test.ts`; `docs/roadmap/referral-saas/ROADMAP.md`; `docs/sa/referral-saas/REFERRAL_SAAS_GAP_MATRIX.md`; `docs/roadmap/ORDERED_TASK_LIST.md`.
Product boundary: Referral SaaS.
Required boundary docs checked: `AGENTS.md`; `docs/product/referral-saas/PRODUCT_BRIEF.md`; `docs/roadmap/referral-saas/ROADMAP.md`; `docs/sa/referral-saas/REFERRAL_SAAS_GAP_MATRIX.md`; `docs/roadmap/ORDERED_TASK_LIST.md`.
Shared primitive impact: Reuses the existing Account Maintenance read model, account registry selector, onboarding readiness projection, Referral SaaS routes, and shared app shell/sidebar primitives; adds an additive account-foundation operating-jurisdiction field used by the product account registry. Source duplication: No.
Linked enhancement: Referral Management and Campaign Attribution SaaS first-wedge productization.
...
Files involved: `dp/migrations/083_referral_saas_account_operating_jurisdiction.sql`; `services/referral_saas_account_foundation_service.py`; `services/referral_saas_account_setup_service.py`; `apps/api/routers/referral_saas_accounts.py`; `frontend/src/api/endpoints/referralSaasAccounts.ts`; `frontend/src/pages/admin/ReferralSaasAccountMaintenancePage.tsx`; `frontend/src/pages/admin/ReferralSaasAccountMaintenancePage.test.tsx`; `frontend/src/app/App.tsx`; `frontend/src/layout/Sidebar.tsx`; `frontend/src/styles/base.css`; relevant backend/frontend tests; roadmap and gap docs.
Database/schema impact: Adds additive `platform_accounts.operating_jurisdiction_code` with bounded values `ZA`, `BW`, `NA`, `ZM`, and `OTHER`, plus a lookup index. Existing rows default to `ZA`; no destructive migration or source fork.
Backend impact: Account creation carries operating jurisdiction from saved company-profile draft evidence where available; account registry read service/API returns safe `operatingJurisdictionCode` without exposing internal tenant identifiers.
Frontend impact: Splits Customer Profile into an entry/customer-search route and a standalone customer profile route, orders sidebar navigation with Account Setup before Customer profile, adds jurisdiction-first customer selection, and keeps customer-home actions scoped to the opened customer.
API impact: Extends existing `GET /v1/referral-saas/accounts` response with `operatingJurisdictionCode`; adds no new backend route.
Tests to add/update: Adds operating-jurisdiction migration test; updates account foundation, account setup, account registry API, frontend endpoint, customer profile page, and sidebar tests.
Validation method: `.venv_codex\Scripts\python.exe -m pytest -q test\test_referral_saas_account_operating_jurisdiction_migration.py test\test_referral_saas_account_foundation_migration.py test\test_referral_saas_account_foundation_service.py test\test_referral_saas_account_setup_service.py test\api\test_referral_saas_accounts_api.py`; `npm.cmd test -- --run src/api/endpoints/referralSaasAccounts.test.ts src/pages/admin/ReferralSaasAccountMaintenancePage.test.tsx src/layout/Sidebar.test.tsx`; `npm.cmd run build`; `git diff --check`.
Acceptance criteria: Customer Profile starts as a country/jurisdiction-first customer finder backed by persisted account jurisdiction; opening a customer navigates to a standalone customer profile page; selected customer profile shows plain-language health, next actions, function cards, and route query context; setup drafts are fallback evidence only; sidebar navigation places Account Setup before Customer profile; internal `tenant_code`, secrets, raw payloads, money, and DLaaS marketplace details are not exposed; no account lifecycle command, durable profile update command, membership write, invitation delivery, credential lifecycle, webhook delivery, campaign activation, go-live, support-case write, repair/replay/retry, reward, funding, fulfilment, settlement, commission, wallet, invoice, payout, sponsor billing, treasury, or broad DLaaS marketplace behavior is added.
...
Risk level: Medium.
Rollback notes: Revert the additive jurisdiction migration, account foundation/setup service updates, account registry API/frontend type updates, Account Maintenance page/test, route, sidebar, style, and roadmap/gap/task-list updates.
...
## TASK-238: Add customer profile settings maintenance command

Status: Complete (2026-07-20). Output: `services/referral_saas_account_foundation_service.py`; `apps/api/routers/referral_saas_accounts.py`; `frontend/src/api/endpoints/referralSaasAccounts.ts`; `frontend/src/pages/admin/ReferralSaasAccountMaintenancePage.tsx`; `docs/sa/referral-saas/REFERRAL_SAAS_PUBLIC_API_CONTRACT_MAP.md`; `docs/sa/referral-saas/REFERRAL_SAAS_ROUTE_SMOKE_INVENTORY.md`; `docs/roadmap/referral-saas/ROADMAP.md`; `docs/sa/referral-saas/REFERRAL_SAAS_GAP_MATRIX.md`; `docs/roadmap/ORDERED_TASK_LIST.md`.
Product boundary: Referral SaaS.
Required boundary docs checked: `AGENTS.md`; `docs/product/referral-saas/PRODUCT_BRIEF.md`; `docs/roadmap/referral-saas/ROADMAP.md`; `docs/sa/referral-saas/REFERRAL_SAAS_GAP_MATRIX.md`; `docs/roadmap/ORDERED_TASK_LIST.md`.
Shared primitive impact: Reuses durable account foundation tables, account audit events, selected Customer Profile route, account registry query, and shared Referral SaaS API client conventions. Source duplication: No.
Linked enhancement: Referral Management and Campaign Attribution SaaS first-wedge productization.
Linked platform/product capability: Customer Profile modular workspace; Customer Settings maintenance; bounded account profile update command.
Objective: Let operators update durable customer profile settings from the selected Customer Profile without rotating customer identifiers, activating accounts, writing memberships, launching campaigns, creating credentials, triggering go-live, or moving money.
Why now: TASK-236 routed Customer Settings into the selected Customer Profile, but the module still behaved like a future placeholder. To support the agreed customer-first operating model, a selected durable customer needs a safe maintenance command for basic profile metadata before campaign deepening continues.
Files involved: `services/referral_saas_account_foundation_service.py`; `apps/api/routers/referral_saas_accounts.py`; `test/test_referral_saas_account_foundation_service.py`; `test/api/test_referral_saas_accounts_api.py`; `scripts/referral_saas_route_smoke_plan.py`; `test/test_referral_saas_route_smoke_inventory.py`; `test/test_referral_saas_route_smoke_plan.py`; `frontend/src/api/endpoints/referralSaasAccounts.ts`; `frontend/src/api/endpoints/referralSaasAccounts.test.ts`; `frontend/src/pages/admin/ReferralSaasAccountMaintenancePage.tsx`; `frontend/src/pages/admin/ReferralSaasAccountMaintenancePage.test.tsx`; roadmap, gap, API contract, and route-smoke docs.
Implementation/source files inspected: `services/referral_saas_account_foundation_service.py`; `services/referral_saas_account_membership_service.py`; `apps/api/routers/referral_saas_accounts.py`; `frontend/src/pages/admin/ReferralSaasAccountMaintenancePage.tsx`; `frontend/src/api/endpoints/referralSaasAccounts.ts`; `docs/product/referral-saas/PRODUCT_BRIEF.md`; `docs/roadmap/referral-saas/ROADMAP.md`; `docs/sa/referral-saas/REFERRAL_SAAS_GAP_MATRIX.md`.
Database/schema impact: No migration. The command updates existing `platform_accounts` profile fields and records `platform_account_audit_events` evidence.
Backend impact: Adds `update_referral_saas_account_profile`, profile validation/guardrails, audit recording, and `PATCH /v1/referral-saas/accounts/{account_ref}/profile`.
Frontend impact: Customer Profile > Customer Settings now shows read-only customer identifiers plus saveable customer name, operating jurisdiction, customer type, and industry fields. The UI confirms the saved update and preserves customer context.
API impact: Adds guarded seeded write `PATCH /v1/referral-saas/accounts/{account_ref}/profile` for bounded profile metadata only.
Tests to add/update: Added service tests, API tests, route smoke inventory/plan coverage, frontend API wrapper tests, and selected Customer Profile settings tests.
Validation method: `.venv_codex\Scripts\python.exe -m pytest -q test\test_referral_saas_account_foundation_service.py test\api\test_referral_saas_accounts_api.py test\test_referral_saas_route_smoke_inventory.py test\test_referral_saas_route_smoke_plan.py`; `npm.cmd test -- --run src/api/endpoints/referralSaasAccounts.test.ts src/pages/admin/ReferralSaasAccountMaintenancePage.test.tsx`; `npm.cmd run build`; `npm.cmd run lint`; `git diff --check`.
Acceptance criteria: Selected Customer Profile can save bounded durable customer profile settings; customer identifiers remain read-only; audit evidence is recorded; route smoke inventory recognizes the new bounded product route; the frontend does not route back to Account Setup for customer profile maintenance; no account activation, external-reference rotation, membership write, invite delivery, seat assignment, auth-claim change, credential lifecycle, webhook delivery, campaign activation, go-live, billing, money movement, DLaaS marketplace behavior, or source-code fork is added.
Dependencies: TASK-236; TASK-237.
Blocked by: None.
Risk level: Medium.
Rollback notes: Remove the profile maintenance service/route/API wrapper, restore Customer Settings as read-only/future work, and revert related tests and docs.
...
## TASK-242: Add Referral SaaS membership activation readiness read model

Status: Complete (2026-07-21). Output: `services/referral_saas_account_membership_service.py`; `apps/api/routers/referral_saas_accounts.py`; `scripts/referral_saas_route_smoke_plan.py`; `test/test_referral_saas_account_membership_service.py`; `test/api/test_referral_saas_accounts_api.py`; `test/test_referral_saas_route_smoke_inventory.py`; `test/test_referral_saas_route_smoke_plan.py`; `frontend/src/api/endpoints/referralSaasAccounts.ts`; `frontend/src/api/endpoints/referralSaasAccounts.test.ts`; `frontend/src/api/queryKeys.ts`; `frontend/src/api/referralSaasAccountQueries.ts`; `frontend/src/pages/admin/ReferralSaasAccountMaintenancePage.tsx`; `frontend/src/pages/admin/ReferralSaasAccountMaintenancePage.test.tsx`; `docs/roadmap/referral-saas/ROADMAP.md`; `docs/sa/referral-saas/REFERRAL_SAAS_GAP_MATRIX.md`; `docs/roadmap/ORDERED_TASK_LIST.md`.
Product boundary: Referral SaaS.
...
Shared primitive impact: Reuses durable account resolution, existing membership posture, role-family summaries, customer-scoped routes, shared API client/query keys, and smoke-route inventory conventions. Source duplication: No.
Linked enhancement: Referral Management and Campaign Attribution SaaS first-wedge productization.
Linked platform/product capability: Customer Profile People and Access; invitation delivery readiness; membership activation readiness; account access guardrails.
Objective: Give operators a customer-scoped read-only view of why invited people are not active yet and what responsibilities or activation prerequisites are still missing.
Why now: TASK-237 and TASK-239 let People and Access record owner/campaign-manager intent, but manual testing showed the UI still did not explain the gap between `INVITED` evidence and usable account access. TASK-242 makes that gap visible without crossing into live invite delivery or identity-provider activation.
Files involved: Membership service/read model, Referral SaaS account API wrapper, route smoke plan/inventory, frontend API/query wiring, Customer Profile People and Access page, tests, roadmap, and gap matrix.
Implementation/source files inspected: `services/referral_saas_account_membership_service.py`; `apps/api/routers/referral_saas_accounts.py`; `frontend/src/pages/admin/ReferralSaasAccountMaintenancePage.tsx`; `frontend/src/api/endpoints/referralSaasAccounts.ts`; `docs/product/referral-saas/PRODUCT_BRIEF.md`; `docs/roadmap/referral-saas/ROADMAP.md`; `docs/sa/referral-saas/REFERRAL_SAAS_GAP_MATRIX.md`; `docs/sa/referral-saas/REFERRAL_SAAS_MEMBERSHIP_ACTIVATION_DELIVERY_BOUNDARY.md`.
Database/schema impact: None.
Backend impact: Adds a read-only activation readiness projection and `GET /v1/referral-saas/accounts/{account_ref}/membership-activation-readiness` with unsafe-scope protection and no writes.
Frontend impact: People and Access now shows activation readiness summary, ready-to-invite/ready-to-activate counts, missing responsibilities, and per-person next actions inside the selected customer page.
API impact: Adds one read-only Referral SaaS product wrapper route; updates smoke route inventory and dry-run smoke plan.
Tests to add/update: Added service tests, API route tests, smoke inventory/plan tests, frontend API wrapper tests, and People and Access rendering/refetch tests.
Validation method: `.venv_codex\Scripts\python.exe -m pytest -q test\test_referral_saas_account_membership_service.py test\api\test_referral_saas_accounts_api.py test\test_referral_saas_route_smoke_inventory.py test\test_referral_saas_route_smoke_plan.py`; `npm.cmd test -- --run src/api/endpoints/referralSaasAccounts.test.ts src/pages/admin/ReferralSaasAccountMaintenancePage.test.tsx`; `npm.cmd run build`; `npm.cmd run lint -- --quiet`; `git diff --check`.
Acceptance criteria: Selected Customer Profile > People and Access explains invited versus active access, missing owner/campaign-manager responsibilities, invite-delivery readiness, activation blockers, and next actions; response redacts internal identifiers; route rejects account-scope mismatch; smoke inventory classifies the route as read-only; frontend refreshes readiness after access intent is recorded; no invite delivery, membership activation, seat assignment, auth/session claim change, account activation, external-reference rotation, credential lifecycle, webhook delivery, campaign activation, go-live, billing, money movement, DLaaS marketplace behavior, or source-code fork is added.
Dependencies: TASK-237; TASK-239; TASK-241.
Blocked by: None.
Risk level: Medium.
Rollback notes: Remove the activation readiness service projection/route/API wrapper and restore People and Access to membership posture only.
...
## TASK-254: Add customer-scoped campaign list and read wrappers

Status: Complete (2026-07-22). Output: `services/referral_saas_campaign_service.py`; `apps/api/routers/referral_saas_accounts.py`; `scripts/referral_saas_route_smoke_plan.py`; `test/api/test_referral_saas_accounts_api.py`; `test/test_referral_saas_route_smoke_inventory.py`; `test/test_referral_saas_route_smoke_plan.py`; `frontend/src/api/endpoints/referralSaasAccounts.ts`; `frontend/src/api/endpoints/referralSaasAccounts.test.ts`; `frontend/src/api/referralSaasAccountQueries.ts`; `frontend/src/api/queryKeys.ts`; `frontend/src/pages/admin/ReferralSaasAccountMaintenancePage.tsx`; `frontend/src/pages/admin/ReferralSaasAccountMaintenancePage.test.tsx`; `docs/sa/referral-saas/REFERRAL_SAAS_ROUTE_SMOKE_INVENTORY.md`; `docs/roadmap/referral-saas/ROADMAP.md`; `docs/sa/referral-saas/REFERRAL_SAAS_GAP_MATRIX.md`; `docs/roadmap/ORDERED_TASK_LIST.md`.
Product boundary: Referral SaaS.
Required boundary docs checked: `AGENTS.md`; `docs/product/referral-saas/PRODUCT_BRIEF.md`; `docs/roadmap/referral-saas/ROADMAP.md`; `docs/sa/referral-saas/REFERRAL_SAAS_GAP_MATRIX.md`; `docs/roadmap/ORDERED_TASK_LIST.md`.
Shared primitive impact: Reuses existing account resolution and campaign tables while keeping tenant identifiers behind the selected customer/account boundary. Source duplication: No.
Linked enhancement: Referral Management and Campaign Attribution SaaS first-wedge productization.
Linked platform/product capability: Customer Profile Campaigns; customer-scoped campaign list/read; tenant-safe campaign operations boundary.
Objective: Add read-only campaign list/read wrappers for a selected Referral SaaS customer and wire the Customer Profile Campaigns page to choose from that list before readiness checks.
Why now: TASK-253 removed tenant-code entry from campaign readiness, but the page still required a manually typed campaign code. Operators need to see campaigns for the selected customer before running readiness checks or starting future campaign setup work.
Files involved: Referral SaaS campaign read model service, account API router, route smoke plan/inventory, frontend endpoint/query wrappers, selected Customer Profile Campaigns page, focused API/page tests, roadmap, and gap matrix.
Database/schema impact: None.
Backend impact: Adds `GET /v1/referral-saas/accounts/{account_ref}/campaigns` and `GET /v1/referral-saas/accounts/{account_ref}/campaigns/{campaign_code}` as read-only account-scoped wrappers over existing campaign tables and policies.
Frontend impact: Selected Customer Profile > Campaigns now loads customer-scoped campaigns, renders selectable campaign rows, defaults readiness to a campaign from that list, and keeps manual campaign-code entry as a fallback.
API impact: Two new read-only Referral SaaS account-scoped campaign routes; no campaign create/update/policy/activation command.
Tests to add/update: Backend API wrapper tests, route smoke inventory/plan tests, frontend endpoint test, and selected Customer Profile Campaigns page test.
Validation method: `.venv_codex\Scripts\python.exe -m pytest -q test\api\test_referral_saas_accounts_api.py test\test_referral_saas_route_smoke_inventory.py test\test_referral_saas_route_smoke_plan.py`; `npm.cmd test -- --run src/api/endpoints/referralSaasAccounts.test.ts src/pages/admin/ReferralSaasAccountMaintenancePage.test.tsx`; `npm.cmd run build`; `npm.cmd run lint -- --quiet`; `git diff --check`.
Acceptance criteria: Customer Profile Campaigns lists only campaigns for the selected account, selected campaign readiness uses account scope and external customer reference, operators do not enter tenant code, internal tenant identifiers remain redacted, and the UI/API confirm no campaign mutation, policy write, link generation, activation, go-live, billing, money movement, DLaaS marketplace behavior, or source-code fork occurred.
Dependencies: TASK-231; TASK-241; TASK-253.
Blocked by: Productized campaign draft/create, campaign policy maintenance, campaign submit/review, activation/go-live command boundaries, and deeper campaign setup UX.
Risk level: Medium.
Rollback notes: Remove the campaign read model service, account campaign list/read routes, frontend endpoint/query/page wiring, tests, smoke-plan entries, and roadmap/gap updates.
Explicit non-goals: Do not create campaigns, update campaign policy, generate links, activate campaigns, trigger go-live, create credentials, send webhooks, send invites, activate memberships, assign seats, change auth/session claims, create reports/exports, bill, move money, implement broad DLaaS marketplace behavior, or fork source code.
...
## TASK-256: Add guarded customer-scoped campaign setup create API wrapper
...
Product boundary: Referral SaaS.
Required boundary docs checked: `AGENTS.md`; `docs/product/referral-saas/PRODUCT_BRIEF.md`; `docs/roadmap/referral-saas/ROADMAP.md`; `docs/sa/referral-saas/REFERRAL_SAAS_GAP_MATRIX.md`; `docs/roadmap/ORDERED_TASK_LIST.md`; `docs/sa/referral-saas/REFERRAL_SAAS_CUSTOMER_CAMPAIGN_CREATE_CONTRACT.md`.
Shared primitive impact: Reuses existing account resolution, campaign storage, and account audit evidence while keeping tenant identifiers behind the selected customer/account boundary. Source duplication: No.
Linked enhancement: Referral Management and Campaign Attribution SaaS first-wedge productization.
Linked platform/product capability: Customer Profile Campaigns; customer-scoped campaign setup create; idempotency; audit evidence; activation guardrails.
Objective: Implement the guarded selected-customer campaign setup create API wrapper defined by TASK-255.
Why now: TASK-254 lets users select existing campaigns from the selected customer profile, but creating a campaign still depended on the generic tenant-scoped campaign primitive that defaults campaigns to active. The SaaS product needs a safe product route that creates only an inactive setup draft before future policy, readiness, review, and activation workflows.
Files involved: Referral SaaS campaign service, account API router, route smoke plan/inventory, API/service tests, public API contract map, roadmap, and gap matrix.
Database/schema impact: None.
Backend impact: Adds `POST /v1/referral-saas/accounts/{account_ref}/campaigns`, resolving selected customer scope internally, creating an inactive `marketing_campaigns` setup draft, recording account audit/idempotency evidence, replaying same-payload idempotency keys, rejecting conflicting idempotency keys, rejecting duplicate campaign name/segment, and rejecting unsafe activation, policy, link, validation-track, webhook, and money fields.
Frontend impact: None in this task; the next task should add the selected-customer campaign create UX against this wrapper.
API impact: New seeded local/staging-only Referral SaaS account-scoped campaign setup create route. It does not activate campaigns, generate links, create validation tracks, write campaign policy, deliver webhooks, trigger go-live, bill, or move money.
Tests to add/update: Campaign service create/idempotency/duplicate tests, API happy-path and unsafe-payload tests, route smoke inventory test, and route smoke plan test.
...
Acceptance criteria: Selected-customer campaign setup can be created only as an inactive draft from account scope, tenant code is not accepted from the caller, idempotency/audit evidence is preserved, duplicate/conflict states are safe, and the response confirms no activation, link generation, validation-track creation, policy write, webhook delivery, money movement, DLaaS marketplace behavior, or source-code fork occurred.
Dependencies: TASK-135; TASK-231; TASK-253; TASK-254; TASK-255.
Blocked by: Selected-customer campaign create UX, campaign policy/settings maintenance, campaign submit/review boundaries, activation/go-live command boundaries, and campaign workflow E2E tests.
Risk level: Medium.
Rollback notes: Remove the campaign setup create service, account route, route smoke entries, tests, and roadmap/gap/public API map updates.
...
## TASK-260: Add selected-customer campaign policy/settings UX

Status: Complete (2026-07-22). Output: `frontend/src/api/client.ts`; `frontend/src/api/endpoints/referralSaasAccounts.ts`; `frontend/src/pages/admin/ReferralSaasAccountMaintenancePage.tsx`; `frontend/src/api/endpoints/referralSaasAccounts.test.ts`; `frontend/src/pages/admin/ReferralSaasAccountMaintenancePage.test.tsx`; `docs/roadmap/referral-saas/ROADMAP.md`; `docs/sa/referral-saas/REFERRAL_SAAS_GAP_MATRIX.md`; `docs/roadmap/ORDERED_TASK_LIST.md`.
Product boundary: Referral SaaS.
Required boundary docs checked: `AGENTS.md`; `docs/product/referral-saas/PRODUCT_BRIEF.md`; `docs/roadmap/referral-saas/ROADMAP.md`; `docs/sa/referral-saas/REFERRAL_SAAS_GAP_MATRIX.md`; `docs/roadmap/ORDERED_TASK_LIST.md`; `docs/sa/referral-saas/REFERRAL_SAAS_CUSTOMER_CAMPAIGN_POLICY_SETTINGS_CONTRACT.md`.
Shared primitive impact: Reuses the selected-customer account context, existing campaign list/readiness query hooks, and the guarded customer-scoped policy/settings API wrapper from TASK-259; extends the shared frontend API client method union to support existing backend `PUT` update routes. Source duplication: No.
Linked enhancement: Referral Management and Campaign Attribution SaaS first-wedge productization.
Linked platform/product capability: Customer Profile Campaigns; customer-scoped campaign policy/settings; attribution settings; campaign readiness; activation guardrails.
Objective: Add the standalone selected-customer Campaign Policy Settings UI that lets operators save campaign policy evidence from inside Customer Profile.
Why now: TASK-259 implemented the guarded backend wrapper, and campaign setup now needs a visible next step after inactive campaign draft creation.
Files involved: Referral SaaS account endpoint client, selected-customer maintenance page, endpoint/page tests, roadmap, gap matrix, and ordered task list.
Database/schema impact: None.
Backend impact: None; consumes existing TASK-259 API.
Frontend impact: Adds `Customer Profile > Campaigns > Policy settings`, campaign-list CTAs to that page, and post-create routing from inactive campaign setup to policy settings.
API impact: Adds a frontend client wrapper for `PUT /v1/referral-saas/accounts/{accountRef}/campaigns/{campaignRef}/policy-settings`.
Tests to add/update: Endpoint wrapper payload/no-leak test and selected-customer page test for standalone policy settings save, campaign selection, success state, and unsafe-field exclusion.
Validation method: `npm.cmd test -- --run src/api/endpoints/referralSaasAccounts.test.ts src/pages/admin/ReferralSaasAccountMaintenancePage.test.tsx`; `npm.cmd run build`; `npm.cmd run lint -- --quiet`; `git diff --check`.
Acceptance criteria: Operators can open policy settings from the selected customer Campaigns page or after campaign setup creation, select a customer campaign, save attribution/eligibility/product-window/terms/reward-visibility evidence, and receive clear confirmation that no tenant code entry, activation, link generation, validation track, webhook delivery, billing, or money movement occurred.
Dependencies: TASK-253; TASK-254; TASK-255; TASK-256; TASK-257; TASK-258; TASK-259.
Blocked by: Campaign submit/review boundary, activation/go-live command boundary, customer-scoped link/code continuation, customer-scoped reporting continuation, and full campaign workflow E2E tests.
Risk level: Medium.
Rollback notes: Remove the endpoint wrapper, policy settings page branch, campaign CTAs, post-create route change, tests, and docs updates.
