...
billing/money side effects. TASK-389 now adds the selected-customer Journey
...
- TASK-378:
  ...
  `frontend/src/pages/admin/ReferralSaasAccountMaintenancePage.tsx`;
  `test/test_referral_saas_reporting_service.py`;
  `test/test_referral_saas_status_reporting_contract.py`;
  `docs/roadmap/ORDERED_TASK_LIST.md`;
  `docs/roadmap/referral-saas/ROADMAP.md`;
  `docs/sa/referral-saas/REFERRAL_SAAS_GAP_MATRIX.md`;
  `outputs/referral-attribution-dlaas-roadmap-infographic.html` - Adds the
  HVE funnel and journey-performance reporting source. Referral funnel now uses
  tenant-scoped referral instances, progress events, campaign attribution links,
  and distribution route links instead of generic overview counts. Operators
  can see validated, attributed, completed, high-value event, attribution-gap,
  and completion-gap metrics with freshness, safe filters, redactions, and
  source warnings while raw UCN, event payload, provider, reward, funding,
  settlement, billing, and money evidence remain excluded. Ratings remain
  9.99/10 for Referral Management and move to 9.99999/10 for Campaign
  Attribution because HVE journey reporting is now source-backed and runtime
  exports have signed-download metadata, expiry, schedule intent, and deletion
...
- **TASK-424 through TASK-430 - Amplifi Global Operations Workspace:** Complete. The shell now separates global
  ...
  until governed source evidence exists. The global dashboard now consumes that evidence for customer
  discovery, authoritative KPIs, a prioritised queue preview, and explainable customer attention without
  synthetic frontend metrics. The dedicated global work queue now adds authoritative server-side
  triage filters, URL-restorable query state, stable cursor pagination, filtered empty states, and
  governed source-workflow links without duplicating the work-item model. The customer portfolio now
  combines the jurisdiction-safe account registry with persisted operational-attention evidence, labelled
  ...
  Physical desktop, tablet, and mobile responsiveness, keyboard traversal, screen-reader spot checks, and
  no-overflow evidence complete the release gate. Placeholder metrics,
