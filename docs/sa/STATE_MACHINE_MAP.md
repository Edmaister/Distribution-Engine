...
| Qualification decision | TASK-013 defines `QUALIFIED`, `NOT_QUALIFIED`, `PENDING`, `BLOCKED`, and `INVALID` decisions over current journey, event, campaign, policy, and audit evidence in `docs/sa/QUALIFICATION_DECISION_CONTRACT.md`. | GAP-07 |
| Money lifecycle | Normalize operator-facing statuses for calculated liability, reserved funding, fulfilled reward/commission, settled amount, reversed/disputed/failed states. TASK-014 defines the reward/commission policy boundary, and TASK-015 defines the liability state model in `docs/sa/LIABILITY_STATE_MODEL.md`. | GAP-08, GAP-09, GAP-10 |
