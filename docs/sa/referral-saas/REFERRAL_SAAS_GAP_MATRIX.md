...
| --- | --- | --- | --- | --- | --- | --- |
| SaaS account packaging | `tenant_code` is used across important flows; admin tenant APIs, onboarding draft persistence, onboarding validation, submit-for-review, review-decision, and permission helpers exist. TASK-190 defines the architecture for real Account Setup, integrated readiness, and Account Maintenance. TASK-191 defines the near-term product wrapper contract over onboarding drafts/readiness. TASK-192 adds the first Account Setup workflow shell, TASK-193 wires it to guarded draft validation/save/submit/review actions, TASK-194 defines the Account Maintenance read model, TASK-195 adds the read-only Account Maintenance shell, TASK-196 adds a safe onboarding draft selector source, TASK-197 completes the additive account schema final review, TASK-198 adds the account foundation migration plus static migration contract tests, TASK-199 adds the read-only external-reference account resolver service, TASK-200 adds the read-only product account resolver API wrapper, and TASK-201 wires Account Setup Step 1 to that durable account resolver. | SaaS customer can onboard company/account, users, roles, setup state, limits, and external references without exposing internal identifiers; existing accounts can be maintained through scoped health, evidence, membership, reference, and audit workflows. | Durable account, organisation, account-tenant, external-reference, user, membership, seat, and account-audit schema exists; resolver service, product read API, and Account Setup frontend resolver wiring can map active/setup external references to safe account context without exposing internal tenant scope. Membership-aware authorization, invitation flows, account lifecycle commands, account maintenance commands, and physical account setup test evidence remain open. | P0 | TASK-201 complete; next task should run local physical Account Setup verification against the live app/API/DB before moving to campaign work | Tenant/account contract tests; API wrapper tests; migration replay tests; onboarding draft wrapper tests; role/membership tests; external-reference resolver tests; tenant isolation tests; maintenance read-model tests; frontend no-fake-command tests; selector no-leak tests; local physical setup evidence. |
