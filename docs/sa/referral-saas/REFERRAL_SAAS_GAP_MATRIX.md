...
| --- | --- | --- | --- | --- | --- | --- |
| SaaS account packaging | `tenant_code` is used across important flows; admin tenant APIs, onboarding draft persistence, onboarding validation, submit-for-review, review-decision, and permission helpers exist. TASK-190 defines the architecture for real Account Setup, integrated readiness, and Account Maintenance. TASK-191 defines the near-term product wrapper contract over onboarding drafts/readiness without faking account creation. | SaaS customer can onboard company/account, users, roles, setup state, limits, and external references without exposing internal `tenant_code`; existing accounts can be maintained through scoped health, evidence, membership, reference, and audit workflows. | Account/user/membership/seat/setup and maintenance model is not productized for Referral SaaS. The product wrapper contract is defined, but implementation, durable account, tenant-link, external-reference mapping, membership, account selector, invitation, lifecycle, and maintenance commands remain open. | P0 | TASK-191: Define Referral SaaS account setup wrapper contract; next task should build the Account Setup workflow shell over existing draft/readiness primitives | Tenant/account contract tests; onboarding draft wrapper tests; role/membership tests; external-reference tests; tenant isolation tests; maintenance read-model tests. |
