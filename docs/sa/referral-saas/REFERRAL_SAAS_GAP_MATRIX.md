...
| Referral code creation | Code creation, preferred handle handling, existing-code reuse, accepted-terms enforcement, and TASK-173 focused issue/reuse UI exist. | Tenant-scoped, documented, auditable issue/reuse flow with clear product API, account-scoped setup UX, and operational evidence. | Core and first UI surface exist; product API wrapper, account/membership scope, audit consistency, and lifecycle operations remain open. | P0 | TASK-173: Add Referral SaaS link/code workflow frontend surface; next task should add bounded product API/account-scope hardening | Duplicate issue tests; terms-required tests; tenant-scope tests; audit/readback tests; frontend no-leak tests. |
| Referral validation and terms | Validation enforces terms, alias rules, referral instance creation, QR scan evidence, safe failures, and TASK-173 focused validation UI exists. | Public validation API has stable errors, idempotency posture, operator trace, recovery UX, and no sensitive leakage. | Validation and first UI surface exist; product API/error/idempotency/operator trace and richer recovery UX still need formalization. | P0 | TASK-173: Add Referral SaaS link/code workflow frontend surface; next task should harden product validation/recovery API behavior | Validation contract tests; duplicate submit tests; safe error tests; QR evidence tests; frontend recovery tests. |
...
| Frontend SaaS workflow | Role-specific React pages and tests exist; TASK-170 account setup, TASK-169/TASK-171 reports/export preview, TASK-172 campaign readiness, and TASK-173 link/code workflow now provide focused Referral SaaS admin surfaces. | Coherent Referral SaaS workflow: account setup, campaign setup, referral link/code management, event/attribution investigation, reporting, safe status. | Focused surfaces now cover setup, campaign readiness, reports, exports preview, and first link/code execution; progress/status, attribution trace, support workflows, and full product shell cohesion remain open. | P1 | TASK-173: Add Referral SaaS link/code workflow frontend surface; next tasks should add progress/status and attribution/support surfaces | Frontend route tests; accessibility tests; no-internal-leak tests; workflow smoke tests. |
