...
| --- | --- | --- | --- | --- | --- | --- |
| SaaS account packaging | `tenant_code` is used across important flows; admin tenant APIs, onboarding draft persistence, onboarding validation, submit-for-review, review-decision, and permission helpers exist. TASK-190 defines the architecture for real Account Setup, integrated readiness, and Account Maintenance. TASK-191 defines the near-term product wrapper contract over onboarding drafts/readiness. TASK-192 adds the first Account Setup workflow shell over existing readiness evidence without faking account creation. | SaaS customer can onboard company/account, users, roles, setup state, limits, and external references without exposing internal `tenant_code`; existing accounts can be maintained through scoped health, evidence, membership, reference, and audit workflows. | Account Setup now has a clearer workflow shell, but draft save/validation/submit/review product wiring, durable account, tenant-link, external-reference mapping, membership, account selector, invitation, lifecycle, and maintenance commands remain open. | P0 | TASK-192: Build Account Setup workflow shell; next task should connect setup workflow to draft save, validation, submit, and review APIs | Tenant/account contract tests; onboarding draft wrapper tests; role/membership tests; external-reference tests; tenant isolation tests; maintenance read-model tests. |
