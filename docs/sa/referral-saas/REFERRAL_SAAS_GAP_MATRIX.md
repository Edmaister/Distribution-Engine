...
| Frontend SaaS workflow | Role-specific React pages and tests exist; TASK-170 through TASK-230 provide the focused Referral SaaS admin/API surfaces, Account Setup wizard, account creation path, account selector, selected Client Workspace, and local physical proof. TASK-231 converts the selected Client Workspace into the intended customer profile model: choose jurisdiction and customer first, then open a standalone selected-customer profile page for customer-scoped campaigns, links/codes, reports, support, attribution, progress, health, and setup functions. TASK-232 makes Account Setup start blank with tooltip-backed identifier guidance and no premature backend lookup. TASK-233 replaces the exposed save-submit-review-create operator ceremony with one Review & Create product action and clearer already-created recovery. TASK-234 removes the hidden `FNB` setup-scope default from Account Setup create and maps internal-scope conflicts to a specific recovery message. TASK-235 removes the final Handoff step so Account Setup terminates at Review & Create and successful creation routes operators to customer-profile-first next actions. TASK-236 keeps customer settings and people/access actions as modules inside the selected Customer Profile route. TASK-237 adds a customer-scoped People and Access form that records guarded access intent without leaving Customer Profile. | Coherent Referral SaaS workflow: account setup, setup checkpoint, customer profile selection, customer home/readiness summary, customer activity routing, campaign setup, technical setup, referral link/code management, event/attribution investigation, reporting, safe status. | Focused surfaces now cover a guided four-step Account Setup wizard, explicit operator-entered customer identifiers, a simplified terminal Review & Create action, no hidden demo/default owner-scope collision, customer-profile-first post-create routing, persisted-jurisdiction-backed customer profile selection, standalone selected customer home, plain-language health at a glance, prioritized next actions, customer-scoped function cards, internal customer modules for settings and people/access, customer-scoped access-intent capture, people/access summary, customer-context route query propagation, setup draft fallback positioning, and ringfenced product navigation. Durable profile update commands, full downstream customer-context consumption, membership activation, delivery provider runtime work, and dedicated Technical Setup productization remain future bounded capabilities before campaign deepening. | P1 | TASK-237 complete; next task should move into membership activation/delivery boundary implementation, durable customer profile maintenance commands, or Account Maintenance command hardening | Frontend route tests; accessibility tests; no-internal-leak tests; setup workflow smoke tests; account create-action tests; membership invitation tests in Account Maintenance; technical setup routing tests; duplicate/already-created state tests; maintenance read-model/workspace tests; local physical setup proof. |
