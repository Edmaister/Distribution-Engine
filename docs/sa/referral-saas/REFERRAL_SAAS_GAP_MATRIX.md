...
| --- | --- | --- | --- | --- | --- | --- |
| SaaS account packaging | `tenant_code` is used across important flows; admin tenant APIs, onboarding draft persistence, onboarding validation, submit-for-review, review-decision, and permission helpers exist. TASK-190 defines the architecture for real Account Setup, integrated readiness, and Account Maintenance. TASK-191 defines the near-term product wrapper contract over onboarding drafts/readiness. TASK-192 adds the first Account Setup workflow shell, TASK-193 wires it to guarded draft validation/save/submit/review actions, TASK-194 defines the Account Maintenance read model, TASK-195 adds the read-only Account Maintenance shell, TASK-196 adds a safe onboarding draft selector source, and TASK-197 completes the additive account schema final review. | SaaS customer can onboard company/account, users, roles, setup state, limits, and external references without exposing internal identifiers; existing accounts can be maintained through scoped health, evidence, membership, reference, and audit workflows. | Account Setup now has workflow and guarded draft action wiring, and Account Maintenance now has a read-only shell plus safe draft selector over onboarding evidence. The schema direction is approved, but durable account, tenant-link, external-reference mapping, membership, invitation, lifecycle, product account API wrappers, and maintenance commands remain open until the additive account foundation is implemented and tested. | P0 | TASK-197 complete; next task should add additive account foundation migration and contract tests | Tenant/account contract tests; migration replay tests; onboarding draft wrapper tests; role/membership tests; external-reference tests; tenant isolation tests; maintenance read-model tests; frontend no-fake-command tests; selector no-leak tests. |
