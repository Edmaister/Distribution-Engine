...
| --- | --- | --- | --- | --- | --- | --- |
| SaaS account packaging | `tenant_code` is used across important flows; admin tenant APIs, onboarding draft persistence, onboarding validation, submit-for-review, review-decision, and permission helpers exist. TASK-190 defines the architecture for real Account Setup, integrated readiness, and Account Maintenance. TASK-191 defines the near-term product wrapper contract over onboarding drafts/readiness. TASK-192 adds the first Account Setup workflow shell, and TASK-193 wires it to guarded draft validation, save, submit-for-review, and internal review-decision actions. | SaaS customer can onboard company/account, users, roles, setup state, limits, and external references without exposing internal `tenant_code`; existing accounts can be maintained through scoped health, evidence, membership, reference, and audit workflows. | Account Setup now has workflow and guarded draft action wiring, but durable account, tenant-link, external-reference mapping, membership, account selector, invitation, lifecycle, product account API wrappers, and maintenance commands remain open. | P0 | TASK-193: Connect Account Setup workflow to draft APIs; next task should define Account Maintenance workflow contract/read model | Tenant/account contract tests; onboarding draft wrapper tests; role/membership tests; external-reference tests; tenant isolation tests; maintenance read-model tests. |
