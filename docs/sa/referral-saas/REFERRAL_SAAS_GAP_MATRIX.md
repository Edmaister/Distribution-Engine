...
| Link/code inspection | Canonical inspection covers referral codes, campaign codes, campaign referral links, route referral links, composite-code compatibility, redactions, missing evidence, and TASK-178 read-only product operator wrapper. | Operator can investigate any SaaS link/code source from safe evidence and jump to related campaign, referral, progress, and attribution state. | Product operator inspection API now exists with next diagnostics, but source-to-trace linking, operator support UI, and deeper progress/attribution workflow links still need packaging. | P1 | TASK-178: Add Referral SaaS operator link/code inspect API wrapper; next task should add operator UI or attribution/support trace wrapper | Admin inspection tests; product wrapper tests; redaction tests; missing source tests; UI workflow tests. |
...
| Public API contracts | Referral, progress, campaign, reward summary, partner-ish APIs exist; TASK-174 adds first link/code product wrappers beside the report/export wrappers, TASK-175 centralizes validation recovery mapping, TASK-176 exposes validation idempotency posture, TASK-177 renders recovery/retry posture in the UI, and TASK-178 adds the first operator diagnostics wrapper. | Versioned Referral SaaS public API with auth, schemas, idempotency, errors, examples, and contract tests. | Reporting/export, link/code, and operator inspect wrappers now exist with tested validation recovery, explicit non-idempotent validation posture, and UI visibility, but campaign/progress/status/attribution/support product wrappers and OpenAPI packaging remain incomplete. | P1 | TASK-178: Add Referral SaaS operator link/code inspect API wrapper; next tasks should add progress/status and attribution/support wrappers | OpenAPI/schema tests; auth tests; idempotency tests; error-shape tests. |
| Frontend SaaS workflow | Role-specific React pages and tests exist; TASK-170 account setup, TASK-169/TASK-171 reports/export preview, TASK-172 campaign readiness, TASK-173 link/code workflow, TASK-174 product link/code API seam, and TASK-177 validation recovery/retry UI now provide focused Referral SaaS admin surfaces. | Coherent Referral SaaS workflow: account setup, campaign setup, referral link/code management, event/attribution investigation, reporting, safe status. | Focused surfaces now cover setup, campaign readiness, reports, exports preview, first product-backed link/code execution, and validation recovery posture; progress/status, attribution trace, support workflows, and full product shell cohesion remain open. | P1 | TASK-177: Add Referral SaaS validation recovery UI; next tasks should add progress/status and attribution/support surfaces | Frontend route tests; accessibility tests; no-internal-leak tests; workflow smoke tests. |
| Operator support workflow | Admin audit, failure, DLQ, enterprise events, campaign readiness, link inspection routes, and TASK-178 read-only product operator inspection wrapper exist. | Operator can resolve validation, progress, link/code, attribution, and reporting issues through safe evidence without DB access. | The first product operator diagnostic API exists, but a cohesive support UI, case workflow, trace linkage, and repair/replay guardrails remain open. | P1 | TASK-145: Define Referral SaaS operator support workflow; next task should add operator support surface or trace wrapper | Support workflow tests; permission tests; redaction tests; evidence-link tests. |
