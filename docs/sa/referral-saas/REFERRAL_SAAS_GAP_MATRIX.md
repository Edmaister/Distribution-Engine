...
| Integrations | Credential/vault lifecycle proof now exists with approved-request version matching, audit readback, runtime/vault/adapter evidence flags, stale-request blocker proof, and no raw secret/provider-dispatch exposure. API/webhook/invite/referral-message provider execution evidence remains. | TASK-368 |
