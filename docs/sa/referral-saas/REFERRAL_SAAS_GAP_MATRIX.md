...
| Tenant-safe reporting | Distribution reporting, materialized views, finance/admin metrics, and tenant-safe analytics service exist in broader repo. | SaaS tenant can report on campaigns, referrals, links/codes, progress events, attribution, conversion, and exports with freshness rules. | Reporting exists by domain, the Referral SaaS report/export contract exists, and TASK-327 defines persisted export file lifecycle boundaries; runtime export file creation/download remains open. | P1 | Implement runtime export file creation/download over TASK-327 | Reporting accuracy tests; tenant filter tests; export tests; freshness tests. |
...
10. Implement runtime export file creation/download over TASK-327.
