...
| Referral validation and terms | Validation enforces terms, alias rules, referral instance creation, QR scan evidence, safe failures, TASK-173 focused validation UI, TASK-174 product validation wrapper, and TASK-175 dedicated validation recovery mapper exist. | Public validation API has stable errors, idempotency posture, operator trace, recovery UX, and no sensitive leakage. | Product wrapper now has centralized, tested safe validation/recovery mapping, but duplicate-submit idempotency, operator trace linkage, and richer frontend recovery UX still need hardening. | P0 | TASK-175: Add Referral SaaS validation recovery mapper; next task should decide duplicate-submit idempotency or add operator trace linkage | Validation contract tests; duplicate submit tests; safe error tests; QR evidence tests; frontend recovery tests. |
...
| Public API contracts | Referral, progress, campaign, reward summary, partner-ish APIs exist; TASK-174 adds first link/code product wrappers beside the report/export wrappers, and TASK-175 centralizes validation recovery mapping. | Versioned Referral SaaS public API with auth, schemas, idempotency, errors, examples, and contract tests. | Reporting/export and link/code wrappers now exist with tested validation recovery shape, but campaign/progress/status/attribution/support product wrappers and OpenAPI packaging remain incomplete. | P1 | TASK-175: Add Referral SaaS validation recovery mapper; next tasks should add progress/status and attribution/support wrappers | OpenAPI/schema tests; auth tests; idempotency tests; error-shape tests. |
