...
| --- | --- | --- | --- | --- | --- | --- |
| SaaS account packaging | `tenant_code` is used across important flows; admin tenant APIs, onboarding draft persistence, onboarding validation, submit-for-review, review-decision, and permission helpers exist. TASK-190 through TASK-230 establish account setup, durable account creation, account registry selection, Client Workspace physical proof, and repeatable fresh-client physical proof. TASK-231 reframes the selected Client Workspace into a customer profile landing experience with customer selection first, customer-home context, plain-language health, next actions, and customer-scoped functions. | SaaS customer can onboard company/account, setup state, limits, and external identifiers without exposing internal identifiers; existing accounts can be maintained through a selected-customer workspace for scoped health, evidence, membership, identifiers, users, roles, technical posture, readiness, activities, dashboards, and audit workflows. | Durable account, organisation, account-tenant, external-reference, user, membership, seat, and account-audit schema exists; account setup and customer selection flows now exist; customer profile landing UX now makes the operator select a customer before working on campaigns, links/codes, reporting, support, attribution, or progress. Product package, billing-plan fields, actual access permissions, invite delivery provider integration, membership activation, seat assignment, auth-claim changes, account lifecycle commands, durable client profile update commands, Account Maintenance user-access write UI, account maintenance commands, and dedicated Technical Setup productization remain open as deliberately bounded future work. | P0 | TASK-231 complete; next account task should move into durable customer profile maintenance commands or Account Maintenance user-access write UX before campaign deepening | Tenant/account contract tests; API wrapper tests; migration replay tests; onboarding draft wrapper tests; role/membership tests; invitation idempotency/audit tests; duplicate membership tests; external-reference resolver tests; tenant isolation tests; maintenance read-model tests; frontend wizard/workspace tests; local physical setup evidence. |
...
| Frontend SaaS workflow | Role-specific React pages and tests exist; TASK-170 through TASK-230 provide the focused Referral SaaS admin/API surfaces, Account Setup wizard, account creation path, account selector, selected Client Workspace, and local physical proof. TASK-231 converts the selected Client Workspace into the intended customer profile landing model: choose customer first, then work inside customer-scoped campaigns, links/codes, reports, support, attribution, progress, health, and setup functions. | Coherent Referral SaaS workflow: account setup, setup checkpoint, customer profile selection, customer home/readiness summary, customer activity routing, campaign setup, technical setup, referral link/code management, event/attribution investigation, reporting, safe status. | Focused surfaces now cover a guided Account Setup wizard, customer profile selection, selected customer home, plain-language health at a glance, prioritized next actions, customer-scoped function cards, people/access summary, customer-context route query propagation, setup draft fallback positioning, and ringfenced product navigation. Account Maintenance user-access write UI, durable profile update commands, full downstream customer-context consumption, membership activation, delivery provider runtime work, and dedicated Technical Setup productization remain future bounded capabilities before campaign deepening. | P1 | TASK-231 complete; next task should move into durable customer profile maintenance commands or Account Maintenance user-access write UX | Frontend route tests; accessibility tests; no-internal-leak tests; setup workflow smoke tests; account create-action tests; membership invitation tests in Account Maintenance; technical setup handoff tests; duplicate/already-created state tests; maintenance read-model/workspace tests; local physical setup proof. |
