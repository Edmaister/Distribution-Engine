...
| Amplifi Global operator pages remained visually denser and wider than the approved prototype | Customer Accounts behavior and data were aligned, but desktop content measure, heading hierarchy, tabs, and section rhythm still diverged | TASK-453 adds shared bounded layout and typography tokens and calibrates Customer Accounts spacing without changing behavior | Closed |
| Create customer remained visually flat after legal identity persistence | Legal identity fields existed, but operators could not distinguish authoritative organisation identity from governed account setup at a glance | TASK-454 groups legal identity and governed setup, clarifies automatic duplicate checks, and preserves the existing command chain | Closed |
| Customer Accounts typography and spacing still visibly diverged from the approved prototype | The legal-identity composition was correct, but the primary application retained a wider measure, larger headings, denser controls, and inconsistent guardrail hierarchy | TASK-455 calibrates shared type/layout tokens and the scoped Customer Accounts creation surface without changing behavior | Closed |
| Customer discovery still diverged in shell treatment, search composition, and real-data result hierarchy | Earlier typography work did not remove debug-oriented global chrome, add the prototype's explicit search action, or protect long persisted identifiers from column overlap | TASK-456 scopes the quiet Amplifi Global shell, adds URL-restorable Search/Clear behavior, and stabilises responsive result columns | Closed |
| Operations landing still used the legacy Referral SaaS lockup and resisted the tablet shell collapse | The approved Amplifi Global shell was scoped to nested Operations routes, excluding the landing route, while selector specificity retained the desktop grid at tablet widths | TASK-457 extends the existing shell classification to the Operations landing and adds explicit tablet/mobile shell overrides | Implementation complete; visual QA blocked |
| Selected-customer home remained diagnostic-heavy and used legacy shell/navigation | Authoritative readiness and actions existed, but the landing hierarchy diverged from the approved progression, priority-action, workspace-destination, and summary composition | TASK-458 re-composes existing evidence and customer-scoped routes inside the Amplifi customer-context shell | Implementation complete; visual QA blocked |
| Active incomplete readiness step was not visually distinct | Completed and current markers shared or lost emphasis while future steps remained pale, making the operator position ambiguous | TASK-459 gives only the first incomplete step an accessible amber marker and semantic current-step state | Implementation complete; visual QA blocked |
| Selected-customer landing terminology diverged from the approved menu | The route and behavior were correct, but the application said Customer overview while the approved customer-context shell says Partner overview | TASK-460 aligns the label while retaining the same account-scoped destination | Implementation complete; visual QA blocked |
| Account setup terminology diverged across the selected-customer journey | The first readiness stage said Account foundation while its corresponding menu destination said Customer profile, breaking the prototype's Account establishment golden thread | TASK-461 aligns both visible labels while retaining authoritative backend evidence, routes, statuses, permissions, and actions | Implementation complete; visual QA blocked |
| Account establishment destination and close-out diverged from the governed journey | The menu and progression terminology matched, but the destination initially lacked the prototype's four-stage evidence journey and later ended by navigating away before refreshing the automated production decision | TASK-462 composes authoritative account, jurisdiction, entitlement, activation, and profile-maintenance evidence, refreshes the automated decision after foundation activation, and returns through Partner overview without implying human approval | Implementation complete; visual QA blocked |
