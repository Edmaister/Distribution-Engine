...
| Campaign attribution trace | Campaign attribution records, track events, referral instances, progress events, campaign referral links, route referral links, journey tests, TASK-139 contract, admin outcome trace, and TASK-180 read-only product attribution trace wrapper exist. | One explainable trace from campaign/link/code/event to attributed outcome, including missing evidence and conflict handling. | Product attribution trace API now exists with first-launch section limits, but the focused trace UI, conflict/precedence UX, progress/status support links, and live E2E evidence remain open. | P0 | TASK-180: Add Referral SaaS operator attribution trace API wrapper; next task should add trace UI or progress/status support diagnostics | Product wrapper tests; golden-path trace tests; missing-evidence tests; conflict tests; cross-tenant tests; UI workflow tests. |
| Link/code inspection | Canonical inspection covers referral codes, campaign codes, campaign referral links, route referral links, composite-code compatibility, redactions, missing evidence, TASK-178 read-only product operator wrapper, TASK-179 focused operator UI, and TASK-180 product attribution trace target. | Operator can investigate any SaaS link/code source from safe evidence and jump to related campaign, referral, progress, and attribution state. | Product operator inspection API, first UI surface, and product attribution trace API now exist, but trace navigation is not yet rendered as a focused UI workflow and deeper progress/status links remain open. | P1 | TASK-180: Add Referral SaaS operator attribution trace API wrapper; next task should add trace UI or progress/status support diagnostics | Admin inspection tests; product wrapper tests; redaction tests; missing source tests; UI workflow tests. |
...
| Frontend SaaS workflow | Role-specific React pages and tests exist; TASK-170 account setup, TASK-169/TASK-171 reports/export preview, TASK-172 campaign readiness, TASK-173 link/code workflow, TASK-174 product link/code API seam, TASK-177 validation recovery/retry UI, TASK-179 operator inspect UI, and TASK-180 attribution trace API now provide focused Referral SaaS admin/API surfaces. | Coherent Referral SaaS workflow: account setup, campaign setup, referral link/code management, event/attribution investigation, reporting, safe status. | Focused surfaces now cover setup, campaign readiness, reports, exports preview, product-backed link/code execution, validation recovery posture, first operator inspection, and trace API readiness; progress/status, attribution trace UI, broader support workflows, and full product shell cohesion remain open. | P1 | TASK-180: Add Referral SaaS operator attribution trace API wrapper; next tasks should add attribution trace UI and progress/status support surfaces | Frontend route tests; accessibility tests; no-internal-leak tests; workflow smoke tests. |
| Operator support workflow | Admin audit, failure, DLQ, enterprise events, campaign readiness, link inspection routes, TASK-178 read-only product operator inspection wrapper, TASK-179 focused operator inspect surface, and TASK-180 read-only product attribution trace wrapper exist. | Operator can resolve validation, progress, link/code, attribution, and reporting issues through safe evidence without DB access. | Product operator diagnostic API/UI and attribution trace API exist, but trace UI, support-case workflow, progress diagnostics, and repair/replay guardrails remain open. | P1 | TASK-180: Add Referral SaaS operator attribution trace API wrapper; next task should add trace UI or progress/status support workflow depth | Support workflow tests; permission tests; redaction tests; evidence-link tests. |
