...
| --- | --- | --- | --- | --- | --- | --- |
| SaaS account packaging | `tenant_code` is used across important flows; admin tenant APIs, onboarding draft persistence, onboarding validation, submit-for-review, review-decision, and permission helpers exist. TASK-190 defines the architecture for real Account Setup, integrated readiness, and Account Maintenance. TASK-191 defines the near-term product wrapper contract over onboarding drafts/readiness. TASK-192 adds the first Account Setup workflow shell, TASK-193 wires it to guarded draft validation/save/submit/review actions, and TASK-194 defines the Account Maintenance read model. | SaaS customer can onboard company/account, users, roles, setup state, limits, and external references without exposing internal `tenant_code`; existing accounts can be maintained through scoped health, evidence, membership, reference, and audit workflows. | Account Setup now has workflow and guarded draft action wiring, and Account Maintenance has a read-model contract, but durable account, tenant-link, external-reference mapping, membership, account selector, invitation, lifecycle, product account API wrappers, and maintenance commands remain open. | P0 | TASK-194: Define Account Maintenance workflow contract/read model; next task should build the read-only Account Maintenance shell | Tenant/account contract tests; onboarding draft wrapper tests; role/membership tests; external-reference tests; tenant isolation tests; maintenance read-model tests. |
