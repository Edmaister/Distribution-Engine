...
| --- | --- | --- | --- | --- | --- | --- |
| SaaS account packaging | `tenant_code` is used across important flows; admin tenant APIs, onboarding draft persistence, onboarding validation, submit-for-review, review-decision, and permission helpers exist. TASK-190 defines the architecture for real Account Setup, integrated readiness, and Account Maintenance. TASK-191 defines the near-term product wrapper contract over onboarding drafts/readiness. TASK-192 adds the first Account Setup workflow shell, TASK-193 wires it to guarded draft validation/save/submit/review actions, TASK-194 defines the Account Maintenance read model, and TASK-195 adds the read-only Account Maintenance shell. | SaaS customer can onboard company/account, users, roles, setup state, limits, and external references without exposing internal identifiers; existing accounts can be maintained through scoped health, evidence, membership, reference, and audit workflows. | Account Setup now has workflow and guarded draft action wiring, and Account Maintenance now has a read-only shell over safe onboarding/readiness evidence, but durable account, tenant-link, external-reference mapping, membership, account selector, invitation, lifecycle, product account API wrappers, and maintenance commands remain open. | P0 | TASK-195: Build Account Maintenance read-only shell; next task should plan/implement account selector or durable account primitive boundary | Tenant/account contract tests; onboarding draft wrapper tests; role/membership tests; external-reference tests; tenant isolation tests; maintenance read-model tests; frontend no-fake-command tests. |
