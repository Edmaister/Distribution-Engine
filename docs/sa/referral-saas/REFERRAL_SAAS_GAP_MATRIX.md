...
| Referral validation and terms | Validation enforces terms, alias rules, referral instance creation, QR scan evidence, safe failures, TASK-173 focused validation UI, and TASK-174 product validation wrapper exist. | Public validation API has stable errors, idempotency posture, operator trace, recovery UX, and no sensitive leakage. | Product wrapper maps safe validation/recovery status, but duplicate-submit idempotency, operator trace linkage, and richer recovery UX still need hardening. | P0 | TASK-174: Add Referral SaaS link/code product API wrappers; next task should harden validation idempotency/recovery behavior | Validation contract tests; duplicate submit tests; safe error tests; QR evidence tests; frontend recovery tests. |
...
| Public API contracts | Referral, progress, campaign, reward summary, partner-ish APIs exist; TASK-174 adds first link/code product wrappers beside the report/export wrappers. | Versioned Referral SaaS public API with auth, schemas, idempotency, errors, examples, and contract tests. | Reporting/export and link/code wrappers now exist, but campaign/progress/status/attribution/support product wrappers and OpenAPI packaging remain incomplete. | P1 | TASK-174: Add Referral SaaS link/code product API wrappers; next tasks should add progress/status and attribution/support wrappers | OpenAPI/schema tests; auth tests; idempotency tests; error-shape tests. |
