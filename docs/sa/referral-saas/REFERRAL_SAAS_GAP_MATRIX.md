...
| Frontend SaaS workflow | Role-specific React pages and tests exist; TASK-170 through TASK-230 provide the focused Referral SaaS admin/API surfaces, Account Setup wizard, account creation path, account selector, selected Client Workspace, and local physical proof. TASK-231 converts the selected Client Workspace into the intended customer profile model: choose jurisdiction and customer first, then open a standalone selected-customer profile page for customer-scoped campaigns, links/codes, reports, support, attribution, progress, health, and setup functions. TASK-232 makes Account Setup start blank with tooltip-backed identifier guidance and no premature backend lookup. TASK-233 replaces the exposed save-submit-review-create operator ceremony with one Review & Create product action and clearer already-created recovery. TASK-234 removes the hidden `FNB` setup-scope default from Account Setup create and maps internal-scope conflicts to a specific recovery message. | Coherent Referral SaaS workflow: account setup, setup checkpoint, customer profile selection, customer home/readiness summary, customer activity routing, campaign setup, technical setup, referral link/code management, event/attribution investigation, reporting, safe status. | Focused surfaces now cover a guided Account Setup wizard, explicit operator-entered customer identifiers, a simplified Review & Create action, no hidden demo/default owner-scope collision, persisted-jurisdiction-backed customer profile selection, standalone selected customer home, plain-language health at a glance, prioritized next actions, customer-scoped function cards, people/access summary, customer-context route query propagation, setup draft fallback positioning, and ringfenced product navigation. Account Maintenance user-access write UI, durable profile update commands, full downstream customer-context consumption, membership activation, delivery provider runtime work, and dedicated Technical Setup productization remain future bounded capabilities before campaign deepening. | P1 | TASK-234 complete; next task should move into durable customer profile maintenance commands or Account Maintenance user-access write UX | Frontend route tests; accessibility tests; no-internal-leak tests; setup workflow smoke tests; account create-action tests; membership invitation tests in Account Maintenance; technical setup handoff tests; duplicate/already-created state tests; maintenance read-model/workspace tests; local physical setup proof. |
