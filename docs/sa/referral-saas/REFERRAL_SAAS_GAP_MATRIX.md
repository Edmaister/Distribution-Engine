...
| Referral validation and terms | Validation enforces terms, alias rules, referral instance creation, QR scan evidence, safe failures, TASK-173 focused validation UI, TASK-174 product validation wrapper, TASK-175 dedicated validation recovery mapper, and TASK-176 explicit idempotency posture exist. | Public validation API has stable errors, idempotency posture, operator trace, recovery UX, and no sensitive leakage. | Product wrapper now has centralized, tested safe validation/recovery mapping and exposes that duplicate submits are not idempotent today; schema-backed idempotent reuse, operator trace linkage, and richer frontend recovery UX still need hardening. | P0 | TASK-176: Expose Referral SaaS validation idempotency posture; next task should implement schema-backed duplicate reuse or add operator trace linkage | Validation contract tests; duplicate submit tests; safe error tests; QR evidence tests; frontend recovery tests. |
...
| Public API contracts | Referral, progress, campaign, reward summary, partner-ish APIs exist; TASK-174 adds first link/code product wrappers beside the report/export wrappers, TASK-175 centralizes validation recovery mapping, and TASK-176 exposes validation idempotency posture. | Versioned Referral SaaS public API with auth, schemas, idempotency, errors, examples, and contract tests. | Reporting/export and link/code wrappers now exist with tested validation recovery and explicit non-idempotent validation posture, but campaign/progress/status/attribution/support product wrappers and OpenAPI packaging remain incomplete. | P1 | TASK-176: Expose Referral SaaS validation idempotency posture; next tasks should add progress/status and attribution/support wrappers | OpenAPI/schema tests; auth tests; idempotency tests; error-shape tests. |
| Frontend SaaS workflow | Role-specific React pages and tests exist; TASK-170 account setup, TASK-169/TASK-171 reports/export preview, TASK-172 campaign readiness, TASK-173 link/code workflow, and TASK-174 product link/code API seam now provide focused Referral SaaS admin surfaces. | Coherent Referral SaaS workflow: account setup, campaign setup, referral link/code management, event/attribution investigation, reporting, safe status. | Focused surfaces now cover setup, campaign readiness, reports, exports preview, and first product-backed link/code execution; progress/status, attribution trace, support workflows, and full product shell cohesion remain open. | P1 | TASK-174: Add Referral SaaS link/code product API wrappers; next tasks should add progress/status and attribution/support surfaces | Frontend route tests; accessibility tests; no-internal-leak tests; workflow smoke tests. |
