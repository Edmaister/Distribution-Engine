...
| --- | --- | --- | --- | --- | --- | --- |
| SaaS account packaging | `tenant_code` is used across important flows; admin tenant APIs, onboarding draft persistence, onboarding validation, submit-for-review, review-decision, and permission helpers exist. TASK-190 through TASK-230 establish account setup, durable account creation, account registry selection, Client Workspace physical proof, and repeatable fresh-client physical proof. TASK-231 adds persisted account operating jurisdiction and reframes the selected Client Workspace into a customer profile model: jurisdiction/customer selection first, then a standalone selected-customer profile route with customer-home context, plain-language health, next actions, and customer-scoped functions. TASK-232 removes demo Account Setup defaults and adds identifier guidance so setup starts from explicit operator-entered customer references. TASK-233 simplifies Review & Create to one primary create action while preserving save, submit, review, idempotency, duplicate-reference, and account-foundation guardrails behind the product action. TASK-234 removes the hidden default `FNB` owner-scope collision by deriving a bounded internal setup seed from customer identifiers, creating/updating that seed inside the guarded account-foundation transaction, and returning a distinct internal-scope duplicate conflict when needed. TASK-235 ends Account Setup at Review & Create and routes successful account-foundation creation to customer-profile-first next actions instead of a separate handoff step. TASK-236 keeps selected-customer people/access and customer settings actions inside Customer Profile modules rather than routing existing customers back into Account Setup. TASK-237 wires People and Access to the existing guarded membership invitation intent API from inside the selected Customer Profile. TASK-238 adds a guarded durable Customer Settings command for account name, operating jurisdiction, customer type, and industry from the selected Customer Profile. TASK-241 splits the selected Customer Profile into a short customer home plus separate customer-scoped pages for health, settings, people/access, campaigns, links/codes, reports, support, attribution, and progress. TASK-242 adds a read-only membership activation readiness model and People and Access panel explaining invite delivery, activation, missing-responsibility, and account/link blockers. TASK-243 adds a customer/account-scoped invitation delivery request boundary that validates invited membership scope, records blocked provider evidence, and returns safe provider-not-configured guidance without sending email. | SaaS customer can onboard company/account, setup state, limits, and external identifiers without exposing internal identifiers; existing accounts can be maintained through a selected-customer workspace for scoped health, evidence, membership, identifiers, users, roles, technical posture, readiness, activities, dashboards, and audit workflows. | Durable account, organisation, account-tenant, external-reference, user, membership, seat, and account-audit schema exists; account setup, persisted-jurisdiction-backed customer selection, standalone customer profile routes, simplified terminal Review & Create, bounded internal seed handling, customer-profile-first post-create routing, selected-customer page routing, customer-scoped access-intent capture, bounded durable customer profile settings updates, read-only membership activation readiness, and a blocked invitation delivery request boundary now exist; customer profile landing UX now makes the operator select a customer before working on campaigns, links/codes, reporting, support, attribution, progress, settings, or people/access, and those functions now open as separate customer pages rather than cluttering the home; Account Setup no longer performs silent demo lookups, no longer makes operators manually drive backend draft/review state transitions, no longer silently reuses `FNB` for every new account foundation, no longer presents handoff as another setup step, and no longer acts as the destination for existing-customer people/access or profile-settings work. Product package, billing-plan fields, actual access permissions, approved invite delivery provider integration, membership activation command, seat assignment, auth-claim changes, account lifecycle commands, external-reference rotation, broader account maintenance commands, and dedicated Technical Setup productization remain open as deliberately bounded future work. | P0 | TASK-243 complete; next account task should move into approved invite delivery provider configuration/integration, membership activation command boundaries, external-reference/account lifecycle boundaries, or Account Maintenance command hardening before campaign deepening | Tenant/account contract tests; API wrapper tests; migration replay tests; onboarding draft wrapper tests; role/membership tests; activation-readiness tests; invitation idempotency/audit tests; duplicate membership tests; profile maintenance command tests; external-reference resolver tests; tenant isolation tests; maintenance read-model tests; frontend wizard/workspace tests; local physical setup evidence. |
...
| Frontend SaaS workflow | Role-specific React pages and tests exist; TASK-170 through TASK-230 provide the focused Referral SaaS admin/API surfaces, Account Setup wizard, account creation path, account selector, selected Client Workspace, and local physical proof. TASK-231 converts the selected Client Workspace into the intended customer profile model: choose jurisdiction and customer first, then open a standalone selected-customer profile page for customer-scoped campaigns, links/codes, reports, support, attribution, progress, health, and setup functions. TASK-232 makes Account Setup start blank with tooltip-backed identifier guidance and no premature backend lookup. TASK-233 replaces the exposed save-submit-review-create operator ceremony with one Review & Create product action and clearer already-created recovery. TASK-234 removes the hidden `FNB` setup-scope default from Account Setup create and maps internal-scope conflicts to a specific recovery message. TASK-235 removes the final Handoff step so Account Setup terminates at Review & Create and successful creation routes operators to customer-profile-first next actions. TASK-236 keeps customer settings and people/access actions as modules inside the selected Customer Profile route. TASK-237 adds a customer-scoped People and Access form that records guarded access intent without leaving Customer Profile. TASK-238 turns Customer Settings into a bounded saveable customer profile module inside the selected Customer Profile. TASK-241 turns stacked customer-home modules into separate customer-scoped pages while preserving customer context. TASK-242 adds People and Access activation readiness copy, counts, missing responsibilities, and next-action guidance. TASK-243 adds the safe invitation delivery request API boundary that can explain provider-not-configured state before live delivery exists. | Coherent Referral SaaS workflow: account setup, setup checkpoint, customer profile selection, customer home/readiness summary, customer activity routing, campaign setup, technical setup, referral link/code management, event/attribution investigation, reporting, safe status. | Focused surfaces now cover a guided four-step Account Setup wizard, explicit operator-entered customer identifiers, a simplified terminal Review & Create action, no hidden demo/default owner-scope collision, customer-profile-first post-create routing, persisted-jurisdiction-backed customer profile selection, standalone selected customer home, plain-language health at a glance, prioritized next actions, customer-scoped function cards, separate customer pages for health, settings, people/access, campaigns, links/codes, reports, support, attribution, and progress, customer-scoped access-intent capture, People and Access activation readiness, safe invitation delivery request boundary, bounded customer profile settings updates, customer-context route query propagation, setup draft fallback positioning, and ringfenced product navigation. Full downstream customer-context consumption, live membership activation, delivery provider runtime work, account lifecycle/reference rotation, and dedicated Technical Setup productization remain future bounded capabilities before campaign deepening. | P1 | TASK-242 complete; next task should move into actual invite delivery provider integration, membership activation command boundaries, account lifecycle/reference-rotation boundaries, or Account Maintenance command hardening | Frontend route tests; accessibility tests; no-internal-leak tests; setup workflow smoke tests; account create-action tests; membership invitation tests in Account Maintenance; activation-readiness UI tests; profile maintenance tests in Customer Profile; technical setup routing tests; duplicate/already-created state tests; maintenance read-model/workspace tests; local physical setup proof. |
