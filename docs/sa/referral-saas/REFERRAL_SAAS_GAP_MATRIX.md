...
| --- | --- | --- | --- | --- | --- | --- |
| SaaS account packaging | `tenant_code` is used across important flows; admin tenant APIs, onboarding draft persistence, onboarding validation, submit-for-review, review-decision, and permission helpers exist. TASK-190 defines the architecture for real Account Setup, integrated readiness, and Account Maintenance. TASK-191 defines the near-term product wrapper contract over onboarding drafts/readiness. TASK-192 adds the first Account Setup workflow shell, TASK-193 wires it to guarded draft validation/save/submit/review actions, TASK-194 defines the Account Maintenance read model, TASK-195 adds the read-only Account Maintenance shell, TASK-196 adds a safe onboarding draft selector source, TASK-197 completes the additive account schema final review, TASK-198 adds the account foundation migration plus static migration contract tests, and TASK-199 adds the read-only external-reference account resolver service. | SaaS customer can onboard company/account, users, roles, setup state, limits, and external references without exposing internal identifiers; existing accounts can be maintained through scoped health, evidence, membership, reference, and audit workflows. | Durable account, organisation, account-tenant, external-reference, user, membership, seat, and account-audit schema exists, and the first read-only resolver can map active external references to account/tenant context with blocked-state handling and safe output redaction. Product account API wrappers, membership-aware authorization, invitation flows, account lifecycle commands, account maintenance commands, and frontend command UX remain open. | P0 | TASK-199 complete; next task should add product account read API wrapper or membership-aware resolver service tests | Tenant/account contract tests; migration replay tests; onboarding draft wrapper tests; role/membership tests; external-reference resolver tests; tenant isolation tests; maintenance read-model tests; frontend no-fake-command tests; selector no-leak tests. |
