...
| Progress and journey checks | Progress events validate identifiers, product/sub-product binding, journey compatibility, self-referral, dedupe key, payload hash, queue emission, and TASK-182 now exposes a read-only operator progress/status diagnostics wrapper over existing dashboard progress and safe-status primitives. | Productized event catalog, clear retry/error classes, tenant diagnostics, replay posture, and visible status updates. | Event ingestion and first support-facing API diagnostics are strong; remaining gaps are progress/status UI, event catalog/OpenAPI packaging, replay posture, and live E2E evidence. | P0 | TASK-182: Add Referral SaaS operator progress/status diagnostics API wrapper; next task should add progress/status support UI | Event contract tests; dedupe/idempotency tests; invalid payload tests; replay/diagnostic tests; E2E status tests. |
| Campaign attribution trace | Campaign attribution records, track events, referral instances, progress events, campaign referral links, route referral links, journey tests, TASK-139 contract, admin outcome trace, TASK-180 read-only product attribution trace wrapper, TASK-181 focused operator trace UI, and TASK-182 progress/status support API exist. | One explainable trace from campaign/link/code/event to attributed outcome, including missing evidence and conflict handling. | Product attribution trace API/UI and progress/status support API now exist, but conflict/precedence UX, progress/status UI links, and live E2E evidence remain open. | P0 | TASK-182: Add Referral SaaS operator progress/status diagnostics API wrapper; next task should add progress/status UI linkage | Product wrapper tests; golden-path trace tests; missing-evidence tests; conflict tests; cross-tenant tests; UI workflow tests. |
| Link/code inspection | Canonical inspection covers referral codes, campaign codes, campaign referral links, route referral links, composite-code compatibility, redactions, missing evidence, TASK-178 read-only product operator wrapper, TASK-179 focused operator UI, TASK-180 product attribution trace target, TASK-181 adjacent trace navigation, and TASK-182 progress/status diagnostics target. | Operator can investigate any SaaS link/code source from safe evidence and jump to related campaign, referral, progress, and attribution state. | Product operator inspection API, first UI surface, product attribution trace API/UI, and progress/status API now exist, but deeper progress/status UI navigation and support-case workflow remain open. | P1 | TASK-182: Add Referral SaaS operator progress/status diagnostics API wrapper; next task should add progress/status support surface | Admin inspection tests; product wrapper tests; redaction tests; missing source tests; UI workflow tests. |
...
| Public API contracts | Referral, progress, campaign, reward summary, partner-ish APIs exist; TASK-174 adds first link/code product wrappers beside the report/export wrappers, TASK-175 centralizes validation recovery mapping, TASK-176 exposes validation idempotency posture, TASK-177 renders recovery/retry posture in the UI, TASK-178 adds the first operator diagnostics wrapper, TASK-180 adds the product attribution trace wrapper, and TASK-182 adds the product progress/status diagnostics wrapper. | Versioned Referral SaaS public API with auth, schemas, idempotency, errors, examples, and contract tests. | Reporting/export, link/code, operator inspect, attribution trace, and progress/status wrappers now exist with tested validation recovery, explicit non-idempotent validation posture, and UI visibility, but campaign/support wrappers and OpenAPI packaging remain incomplete. | P1 | TASK-182: Add Referral SaaS operator progress/status diagnostics API wrapper; next tasks should add progress/status UI and OpenAPI packaging | OpenAPI/schema tests; auth tests; idempotency tests; error-shape tests. |
| Frontend SaaS workflow | Role-specific React pages and tests exist; TASK-170 account setup, TASK-169/TASK-171 reports/export preview, TASK-172 campaign readiness, TASK-173 link/code workflow, TASK-174 product link/code API seam, TASK-177 validation recovery/retry UI, TASK-179 operator inspect UI, TASK-180 attribution trace API, TASK-181 attribution trace UI, and TASK-182 progress/status API now provide focused Referral SaaS admin/API surfaces. | Coherent Referral SaaS workflow: account setup, campaign setup, referral link/code management, event/attribution investigation, reporting, safe status. | Focused surfaces now cover setup, campaign readiness, reports, exports preview, product-backed link/code execution, validation recovery posture, first operator inspection, attribution trace, and a progress/status API; progress/status UI, broader support workflows, and full product shell cohesion remain open. | P1 | TASK-182: Add Referral SaaS operator progress/status diagnostics API wrapper; next tasks should add progress/status support surfaces | Frontend route tests; accessibility tests; no-internal-leak tests; workflow smoke tests. |
| Operator support workflow | Admin audit, failure, DLQ, enterprise events, campaign readiness, link inspection routes, TASK-178 read-only product operator inspection wrapper, TASK-179 focused operator inspect surface, TASK-180 read-only product attribution trace wrapper, TASK-181 focused trace UI, and TASK-182 read-only progress/status diagnostics wrapper exist. | Operator can resolve validation, progress, link/code, attribution, and reporting issues through safe evidence without DB access. | Product operator diagnostic API/UI, attribution trace API/UI, and progress/status API exist, but support-case workflow, progress/status UI, and repair/replay guardrails remain open. | P1 | TASK-182: Add Referral SaaS operator progress/status diagnostics API wrapper; next task should add progress/status support workflow depth | Support workflow tests; permission tests; redaction tests; evidence-link tests. |
