...
| --- | --- | --- | --- | --- | --- | --- |
| SaaS account packaging | `tenant_code` is used across important flows; admin tenant APIs, onboarding draft persistence, onboarding validation, submit-for-review, review-decision, and permission helpers exist. TASK-190 defines the architecture for real Account Setup, integrated readiness, and Account Maintenance. TASK-191 defines the near-term product wrapper contract over onboarding drafts/readiness. TASK-192 adds the first Account Setup workflow shell, TASK-193 wires it to guarded draft validation/save/submit/review actions, TASK-194 defines the Account Maintenance read model, TASK-195 adds the read-only Account Maintenance shell, TASK-196 adds a safe onboarding draft selector source, TASK-197 completes the additive account schema final review, and TASK-198 adds the account foundation migration plus static migration contract tests. | SaaS customer can onboard company/account, users, roles, setup state, limits, and external references without exposing internal identifiers; existing accounts can be maintained through scoped health, evidence, membership, reference, and audit workflows. | Durable account, organisation, account-tenant, external-reference, user, membership, seat, and account-audit schema now exists as an additive foundation. Product account API wrappers, resolver service behavior, invitation flows, account lifecycle commands, account maintenance commands, and frontend command UX remain open. | P0 | TASK-198 complete; next task should add account foundation service/read model or external-reference resolver contract tests | Tenant/account contract tests; migration replay tests; onboarding draft wrapper tests; role/membership tests; external-reference resolver tests; tenant isolation tests; maintenance read-model tests; frontend no-fake-command tests; selector no-leak tests. |
