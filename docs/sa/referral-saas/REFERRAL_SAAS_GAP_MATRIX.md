...
| Link/code inspection | Canonical inspection covers referral codes, campaign codes, campaign referral links, route referral links, composite-code compatibility, redactions, missing evidence, TASK-178 read-only product operator wrapper, and TASK-179 focused operator UI. | Operator can investigate any SaaS link/code source from safe evidence and jump to related campaign, referral, progress, and attribution state. | Product operator inspection API and first UI surface now exist, but trace navigation is still link-only/adjacent, and deeper progress/attribution workflow links remain open. | P1 | TASK-179: Add Referral SaaS operator link/code inspect frontend surface; next task should add attribution trace wrapper or support workflow depth | Admin inspection tests; product wrapper tests; redaction tests; missing source tests; UI workflow tests. |
...
| Frontend SaaS workflow | Role-specific React pages and tests exist; TASK-170 account setup, TASK-169/TASK-171 reports/export preview, TASK-172 campaign readiness, TASK-173 link/code workflow, TASK-174 product link/code API seam, TASK-177 validation recovery/retry UI, and TASK-179 operator inspect UI now provide focused Referral SaaS admin surfaces. | Coherent Referral SaaS workflow: account setup, campaign setup, referral link/code management, event/attribution investigation, reporting, safe status. | Focused surfaces now cover setup, campaign readiness, reports, exports preview, product-backed link/code execution, validation recovery posture, and first operator inspection; progress/status, attribution trace, broader support workflows, and full product shell cohesion remain open. | P1 | TASK-179: Add Referral SaaS operator link/code inspect frontend surface; next tasks should add progress/status and attribution/support surfaces | Frontend route tests; accessibility tests; no-internal-leak tests; workflow smoke tests. |
| Operator support workflow | Admin audit, failure, DLQ, enterprise events, campaign readiness, link inspection routes, TASK-178 read-only product operator inspection wrapper, and TASK-179 focused operator inspect surface exist. | Operator can resolve validation, progress, link/code, attribution, and reporting issues through safe evidence without DB access. | The first product operator diagnostic API and UI exist, but support-case workflow, attribution trace linkage, progress diagnostics, and repair/replay guardrails remain open. | P1 | TASK-179: Add Referral SaaS operator link/code inspect frontend surface; next task should add attribution trace wrapper or support workflow depth | Support workflow tests; permission tests; redaction tests; evidence-link tests. |
