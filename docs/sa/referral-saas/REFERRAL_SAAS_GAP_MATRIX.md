...
| Link/code inspection | Canonical inspection covers referral codes, campaign codes, campaign referral links, route referral links, composite-code compatibility, redactions, missing evidence, TASK-178 read-only product operator wrapper, TASK-179 focused operator UI, TASK-180 product attribution trace target, TASK-181 adjacent trace navigation, TASK-182 progress/status diagnostics target, TASK-183 progress/status UI navigation, TASK-184 support hub triage, TASK-295 support-case persistence contract, TASK-297 support-case create/list/read API persistence, TASK-321 selected-customer support-case UI, and TASK-322 support-case notes/status API. | Operator can investigate any SaaS link/code source from safe evidence, jump to related campaign, referral, progress, and attribution state, and persist a selected-customer support case with safe evidence links. | Product operator inspection API/UI, product attribution trace API/UI, progress/status API/UI, support triage hub, support-case contract, runtime support-case create/list/read schema/API, and selected-customer support-case list/create UI, backend notes/status APIs, and selected-customer lifecycle UI wiring now exist, but aggregate queues and repair/replay guardrails remain open. | P1 | Add aggregate support queue after selected-customer lifecycle UI | Admin inspection tests; product wrapper tests; redaction tests; missing source tests; UI workflow tests; support-case evidence-link tests; support-case UI tests. |
...
aggregate support queues, export file storage/download/retention, and progress/attribution
mutation proof beyond campaign report/export preview.
...
notes and change bounded case statuses from the customer Support page. Aggregate
support queues, repair/replay guardrails, export file storage/download, and
progress/attribution mutation proof remain separate future gaps.
