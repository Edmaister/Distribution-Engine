...
| Governed invitation delivery, auth/login completion | TASK-345 to TASK-347; TASK-363 | TASK-345 contract, TASK-346 guarded API boundary, TASK-347 People and Access UI wiring, and TASK-363 live provider-backed invite delivery are complete. Operators can send invite email through the approved shared channel provider after recipient/provider/template/idempotency/audit/redaction gates pass, see sent/failed/blocked evidence, and still keep acceptance, seats, login credentials, auth claims, campaign activation, billing, money, DLaaS, and source forks separated. |
