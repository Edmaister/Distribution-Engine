...
| `/v1/referral-saas/accounts/{accountRef}/journey-drafts/{draftRef}/validate` | `POST` | TASK-386 wrapper over TASK-384 validation result schema | Amplifi Admin/account reader bridge | Implemented as guarded selected-customer draft validation. Records validation evidence and draft validation status only; richer validation/simulation remains TASK-387. No runtime mutation, campaign binding, campaign activation, provider dispatch, auth, billing, or money behavior. |
