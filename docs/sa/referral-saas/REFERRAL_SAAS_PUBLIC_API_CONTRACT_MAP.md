...
| `/v1/referral-saas/accounts/{accountRef}/journey-drafts` | `GET` | TASK-386 wrapper over TASK-384 journey draft schema | Amplifi Admin/account reader bridge | Implemented as selected-customer journey draft list. Resolves account scope from external references, returns safe draft summaries, and does not create, validate, publish, bind campaigns, execute journeys, dispatch providers, change auth, bill, or move money. |
| `/v1/referral-saas/accounts/{accountRef}/journey-drafts` | `PUT` | TASK-386 wrapper over TASK-384 journey draft schema | Amplifi Admin/account reader bridge | Implemented as guarded selected-customer draft save/update. Requires approved global template version, selected account scope, idempotency, payload hashing, unsafe-payload rejection, audit evidence, guardrails, and redactions. Does not publish, bind campaigns, execute journeys, dispatch providers, change auth, bill, or move money. |
| `/v1/referral-saas/accounts/{accountRef}/journey-drafts/{draftRef}/validate` | `POST` | TASK-386 wrapper over TASK-384 validation result schema; TASK-387 validation/simulation service | Amplifi Admin/account reader bridge | Implemented as guarded selected-customer draft validation. Records validation evidence, draft validation status, blocker/warning evidence, and a safe simulation summary for approved template milestones, transitions, evidence, reward safety, and attribution safety. No runtime mutation, campaign binding, campaign activation, provider dispatch, auth, billing, or money behavior. |
...
| `/v1/referral-saas/accounts/{accountRef}/memberships/{membershipRef}/login-completion-readiness` | `GET` | TASK-345 contract plus TASK-346 governed auth/login completion API boundary | Account admin bridge or future login setup permission | Implemented safe read. Shows whether an accepted/provisioned person is ready for actual platform login without creating credentials, sending invites, changing auth claims, activating campaigns, billing, or moving money. |
| `/v1/referral-saas/accounts/{accountRef}/memberships/{membershipRef}/login-completion-intents` | `POST` | TASK-345 contract plus TASK-346 governed auth/login completion API boundary | Account admin bridge or future login setup permission | Implemented guarded command. Records login completion intent/evidence only after active account, active membership, seat-policy, identity-provider, permission-profile, audit, idempotency, and redaction gates pass. It rejects raw passwords, tokens, arbitrary auth claims, internal tenant codes, invite delivery, campaign activation, billing, money movement, and DLaaS behavior. |
| `/v1/referral-saas/accounts/{accountRef}/identity-login-reconciliation` | `GET` | TASK-365 identity/login reconciliation wrapper | Account admin bridge or future login setup permission | Implemented safe read. Reconciles all named/accepted customer access people against optional platform seat state, identity-provider evidence, revocation posture, and auth-claim readiness. It does not send invites, create credentials, assign seats, mutate auth claims, activate campaigns, trigger go-live, bill, move money, expose internal tenant codes, or mutate DLaaS records. |
...
| `/v1/referral-saas/accounts/{accountRef}/integrations/credential-requests/{requestRef}/execution-checks` | `POST` | TASK-314 contract plus TASK-319 runtime API foundation; TASK-320 frontend wiring | SaaS account admin/operator bridge or future customer integration admin | Implemented as safe execution-readiness evidence after approved review. Requires selected-customer scope, correlation, idempotency, audit evidence, unsafe-payload rejection, and no credential creation/storage/reveal/download/rotation/revoke, provider call, vault write, webhook dispatch, invite/message delivery, auth/session change, campaign activation, billing, or money movement. The selected-customer Integrations UI now exposes this as a `Check approved setup` action. Provider/vault execution remains a separate governed task. |
...
| `/v1/referral-saas/accounts/{accountRef}/support-cases` | `POST` | TASK-297 implementation of TASK-295 contract; TASK-321 frontend wiring | SaaS account admin/operator bridge | Implemented selected-customer support-case create command. The selected-customer Support UI now records safe cases with category, priority, title, summary, optional safe evidence reference, idempotency/correlation, and plain-language feedback. Resolves account scope internally, records idempotency/audit evidence, links only safe diagnostic evidence, and rejects raw UCNs, provider payloads, secrets, tokens, repair/replay/retry, campaign activation, export file creation, invite delivery, credential/auth changes, billing, money movement, and DLaaS marketplace side effects. |
| `/v1/referral-saas/accounts/{accountRef}/support-cases` | `GET` | TASK-297 implementation of TASK-295 contract; TASK-321 frontend wiring | SaaS account admin/operator bridge | Implemented selected-customer support-case list. The selected-customer Support UI now displays customer-scoped cases and safe evidence-link counts. Returns only customer-scoped cases and safe evidence links. |
...
| `/v1/referral-saas/accounts/{accountRef}/support-cases/{caseRef}/notes` | `POST` | TASK-322 implementation of TASK-295 lifecycle contract | SaaS account admin/operator bridge | Implemented selected-customer support-case note command. Adds bounded note types, idempotency/payload replay, audit, redactions, selected-customer path scope, and no repair/replay/retry, referral/campaign/progress/attribution/report/export, invite, credential/auth, billing, money, or DLaaS side effects. |
| `/v1/referral-saas/accounts/{accountRef}/support-cases/{caseRef}/status` | `POST` | TASK-322 implementation of TASK-295 lifecycle contract | SaaS account admin/operator bridge | Implemented selected-customer support-case status transition command. Adds bounded statuses, reason capture, idempotency/payload replay, audit, redactions, selected-customer path scope, and the same no-adjacent-action guardrails as support-case notes. |
| `/v1/referral-saas/accounts/{accountRef}/support-cases/{caseRef}/repair-replay-readiness` | `GET` | TASK-336 contract; TASK-337 runtime | SaaS account admin/operator bridge | Implemented read-only selected-customer repair/replay readiness route. Classifies diagnostic, governed repair, governed replay, and hard-excluded posture; returns required evidence, owning workflow, redactions, blocked reasons, and safe next posture; and does not perform repair, replay, retry, provider dispatch, credential/auth change, campaign activation, billing, money movement, broad DLaaS action, or generic DB mutation. |
| `/v1/referral-saas/operator/support-cases` | `GET` | TASK-324 aggregate queue contract over TASK-295/TASK-297/TASK-322 selected-customer persistence boundary and TASK-323 lifecycle UI | Operator/support role | Future read-only operator aggregate queue. Contract now defines safe filters, queue item fields, ordering, redactions, and no-adjacent-action guardrails. Implementation remains separate. |
