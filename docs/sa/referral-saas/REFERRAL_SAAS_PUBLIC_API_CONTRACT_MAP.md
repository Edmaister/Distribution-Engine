...
| `/v1/referral-saas/accounts/{accountRef}/memberships/{membershipRef}/login-completion-readiness` | `GET` | TASK-345 governed auth/login completion contract | Account admin bridge or future login setup permission | Future safe read. Shows whether an accepted/provisioned person is ready for actual platform login without creating credentials, sending invites, changing auth claims, activating campaigns, billing, or moving money. |
| `/v1/referral-saas/accounts/{accountRef}/memberships/{membershipRef}/login-completion-intents` | `POST` | TASK-345 governed auth/login completion contract | Account admin bridge or future login setup permission | Future guarded command. Records login completion intent/evidence only after active account, active membership, seat-policy, identity-provider, permission-profile, audit, idempotency, and redaction gates pass. It must reject raw passwords, tokens, arbitrary auth claims, internal tenant codes, invite delivery, campaign activation, billing, money movement, and DLaaS behavior. |
