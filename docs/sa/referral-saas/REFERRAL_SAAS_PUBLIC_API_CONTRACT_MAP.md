...
| `/v1/referral-saas/accounts/{accountRef}/support-cases` | `POST` | TASK-295 contract | SaaS account admin/operator bridge | Target selected-customer support-case create command. Must resolve account scope internally, require idempotency/audit evidence, link only safe diagnostic evidence, and avoid repair/replay/retry, campaign activation, export file creation, invite delivery, credential/auth changes, billing, and money movement. |
| `/v1/referral-saas/accounts/{accountRef}/support-cases` | `GET` | TASK-295 contract | SaaS account admin/operator bridge | Target selected-customer support-case list. Must return only customer-scoped cases and safe evidence links. |
| `/v1/referral-saas/accounts/{accountRef}/support-cases/{caseRef}` | `GET` | TASK-295 contract | SaaS account admin/operator bridge | Target selected-customer support-case read. Must not expose raw diagnostic payloads, secrets, tokens, UCNs, DLQ payloads, or cross-tenant evidence. |
| `/v1/referral-saas/operator/support-cases` | `GET` | TASK-145 future contract; TASK-295 selected-customer persistence contract | Operator/support role | Future operator aggregate only. First implementation should start from selected-customer account routes. |
