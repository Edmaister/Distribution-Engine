...
| `/v1/referral-saas/accounts/{accountRef}/campaigns/{campaignRef}/policy-settings` | `PUT` | TASK-258 contract over existing campaign policy table/service | SaaS account admin bridge | Contract only. Must resolve selected customer account and campaign scope internally, persist policy/settings evidence without caller-supplied tenant code, and must not activate campaigns, generate links, create validation tracks, deliver webhooks, bill, or move money. |
