...
| `/v1/referral-saas/accounts/{accountRef}/support-cases/{caseRef}/repair-replay-readiness` | `GET` | TASK-336 contract; TASK-337 future runtime | SaaS account admin/operator bridge | Future read-only selected-customer repair/replay readiness route. It must classify the case action posture without performing repair, replay, retry, provider dispatch, credential/auth change, campaign activation, billing, money movement, broad DLaaS action, or generic DB mutation. |
