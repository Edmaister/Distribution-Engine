...
- `REFERRAL_SAAS_CUSTOMER_CAMPAIGN_SUBMIT_REVIEW_CONTRACT.md` - TASK-261
  customer-scoped campaign submit/review command boundary.
- `REFERRAL_SAAS_CUSTOMER_CAMPAIGN_ACTIVATION_CONTRACT.md` - TASK-264
  customer-scoped campaign activation/go-live command boundary.
...
- `REFERRAL_SAAS_ATTRIBUTION_TRACE_CONTRACT.md` - TASK-139 attribution trace
  contract.
- `REFERRAL_SAAS_PROGRESS_ATTRIBUTION_MUTATION_PROOF_CONTRACT.md` - TASK-339
  selected-customer progress and attribution mutation proof contract.
...
- `REFERRAL_SAAS_PROVIDER_VAULT_RUNTIME_ADAPTER_CONTRACT.md` - TASK-342
  ...
  SaaS Integrations.
- `REFERRAL_SAAS_PROVIDER_VAULT_RUNTIME_EXECUTION_COMMAND_CONTRACT.md` -
  TASK-349 governed provider/vault runtime execution command contract for the
  controlled path from approved credential request to future opaque execution
  references.
...
- `REFERRAL_SAAS_PLATFORM_REFERENCE_PROVIDER_VAULT_ADAPTERS.md` - TASK-353
  first safe runtime `PLATFORM_REFERENCE` and `PLATFORM_VAULT_REFERENCE`
  adapters that return opaque execution references without raw secrets or
  vendor dispatch.
- `REFERRAL_SAAS_VENDOR_MANAGED_PROVIDER_VAULT_ADAPTER_CONTRACT.md` - TASK-354
  vendor/managed provider-vault adapter contract after the platform-reference
  adapter.
- `REFERRAL_SAAS_VENDOR_MANAGED_PROVIDER_VAULT_ADAPTERS.md` - TASK-355
  first built-in vendor/managed provider-vault runtime adapter path with
  explicit provider allowlist, managed-vault configuration, opaque references,
  and no raw secret or vendor dispatch side effects.
...
- `REFERRAL_SAAS_SUPPORT_QUEUE_CONTRACT.md` - TASK-324 operator aggregate
...
- TASK-186 adds explicit local testing guidance to the focused workspace and
  account setup surface. It answers what each screen is for, what can be done
