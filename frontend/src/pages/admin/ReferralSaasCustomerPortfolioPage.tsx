...
const destinations = {
  programmes: {
    title: "Select a customer for programme governance",
    copy: "Choose a customer within your permitted jurisdictions, then manage that customer's governed programme versions.",
    action: "Open programmes",
    suffix: "programmes",
  },
  commercial: {
    title: "Select a customer for commercial governance",
    copy: "Choose a customer within your permitted jurisdictions, then review that customer's commercial entitlement and billing posture.",
    action: "Open commercial governance",
    suffix: "commercial",
  },
} as const;
...
export function ReferralSaasCustomerPortfolioPage() {
  ...
  const destinationKey = value(params, "destination");
  const destination = destinationKey && destinationKey in destinations
    ? destinations[destinationKey as keyof typeof destinations]
    : undefined;
  ...
  const hasFilters = [...params.keys()].some((key) => !["destination", "limit", "sort"].includes(key));
  ...
  return <div className="operations-workspace customer-portfolio-page">
    <section className="page-header operations-hero">
      <div>
        ...
        <h1 className="page-title">{destination?.title || "Customer portfolio"}</h1>
        <p className="page-copy">{destination?.copy || "Find a customer within your permitted jurisdictions, understand what needs attention, and open the correct customer workspace."}</p>
      </div>
    ...
    <section className="panel operations-filter-panel" aria-label="Customer portfolio filters">
      <div className="panel-header">
        ...
        {hasFilters ? <button className="button secondary" onClick={() => setParams({ ...(destinationKey ? { destination: destinationKey } : {}), sort: filters.sort || "ATTENTION", limit: String(limit) })} type="button"><X size={15} /> Clear filters</button> : null}
      </div>
    ...
    {portfolio ? <section className="panel customer-portfolio-results">
      ...
      {portfolio.customers.length ? <div className="customer-portfolio-list" role="list">{portfolio.customers.map((customer) => <CustomerRow customer={customer} destination={destination} key={customer.accountRef} />)}</div> : <EmptyState label={hasFilters ? "No customers match these filters. Clear or change a filter to widen the portfolio." : "No customers are available in your permitted scope."} />}
...
function CustomerRow({ customer, destination }: { customer: Customer; destination?: (typeof destinations)[keyof typeof destinations] }) {
  ...
  return <article className="customer-portfolio-row" role="listitem">
    ...
    <Link className="button secondary customer-portfolio-open" to={destination ? `${customer.destination}/${destination.suffix}` : customer.destination}>{destination?.action || "Open customer"} <ArrowRight size={15} /></Link>
  </article>;
}
