...
const checklistItems: ChecklistItem[] = [
  {
    ...
    evidence: "External tenant and organisation references can be drafted in the company onboarding shell.",
    blocker: "Backend account lifecycle APIs are still future work.",
    nextAction: "Confirm organisation_ref, external_tenant_ref, country, industry, and admin contact.",
  },
  {
    ...
    evidence: "Producer/sponsor identity, ownership, and funding intention can be captured locally.",
    blocker: "No sponsor create, wallet, billing, funding contract, or funding readiness command is wired.",
    nextAction: "Draft producer_ref, sponsor_ref, organisation_ref, admin contact, and funding model intention.",
  },
  {
    ...
    evidence: "Distributor identity, channel, market, and participation intent can be captured locally.",
    blocker: "No distributor create, route activation, wallet creation, or marketplace lifecycle command is wired.",
    nextAction: "Draft distributor_ref, organisation_ref, channel type, market, and distributor admin contact.",
  },
  {
    ...
    evidence: "Invite, membership, role-family, participant type, and access scope intent can be drafted.",
    blocker: "No user invite, role assignment, membership activation, or auth claim change is wired.",
    nextAction: "Confirm intended role families for platform operator, producer/company admin, and distributor/partner admin personas.",
  },
  {
    ...
    evidence: "Campaign basics, participant scope, distribution model, outcome, reward, and funding intent can be reviewed.",
    blocker: "No campaign create, opportunity publish, link/code generation, policy write, or launch command is wired.",
    nextAction: "Draft campaign_code, opportunity_ref, outcome event, reward/commission intent, and funding intention.",
  },
  {
    ...
    evidence: "Integration owner, callback placeholder, event categories, auth method intent, and payload preview can be reviewed.",
    blocker: "No API keys, secrets, subscriptions, signing, callback registration, or webhook delivery are created.",
    nextAction: "Confirm selected event categories, callback URL placeholder, payload format, and security method intent.",
  },
  {
    ...
    path: "/admin/onboarding/members-roles",
    pathLabel: "User & role setup",
    evidence: "Permission boundaries are visible in the role setup shell and API permission matrix.",
    blocker: "Membership APIs, live DB verification, and TASK-027/TASK-028 drift checks remain blocked before release signoff.",
    nextAction: "Keep tenant_code internal, use external references, and complete approved read-only live verification before external release.",
  },
  {
    ...
    blocker: "Live activation, campaign publication, funding readiness commands, webhook delivery checks, TASK-027, and TASK-028 are blocked.",
    nextAction: "Use this checklist for demo review only; request implementation tasks for real activation workflows later.",
  },
];

const statusOrder: ReadinessStatus[] = ["Ready", "In progress", "Blocked", "Not started"];
  ...
  return (
    <>
      ...
      <section className="grid-3">
        <SummaryItem label="Ready categories" value={`${readyCount}/8`} />
      ...
      <section className="banner warning" role="note">
        <ShieldCheck size={18} />
        <div>
          <strong>This checklist does not activate go-live, publish campaigns, create credentials, or move money.</strong>
          ...
          <div className="panel-body route-list">
            ...
            <div className="route-item">
              <div>
                ...
                <div className="route-path">
                  Go-live review remains disabled until real account,
                  campaign, credential, funding, fulfilment, settlement, and
                  audit workflows are separately implemented and tested.
                </div>
              </div>
        ...
        <section className="panel">
          <div className="panel-header">
            <div>
              ...
              <div className="panel-subtitle">
                Local/demo readiness categories only.
              </div>
            </div>
            <Rocket size={18} />
          </div>
          <div className="panel-body route-list">
            {statusOrder.map((status) => {
              const count = checklistItems.filter((item) => item.status === status).length;
              return (
                <div className="route-item" key={status}>
                  <div>
                    <div className="route-name">{status}</div>
                    <div className="route-path">{count} checklist categories</div>
                  </div>
        ...
        <section className="panel">
          <div className="panel-header">
            <div>
              ...
              <div className="panel-subtitle">
                External identifiers stay visible; internal partitioning stays hidden.
              </div>
            </div>
            <KeyRound size={18} />
          </div>
